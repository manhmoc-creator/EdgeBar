...
/**
 * V19.12.3.6.8 THE ETERNAL EGO — Tile3
...
@Override public void onClick() {
    super.onClick();
    SharedPreferences prefs = getSharedPreferences("EdgeBarPrefs", MODE_PRIVATE);
    String act = prefs.getString("tile_3_act", "NONE");
    if (!act.equals("NONE")) {
        Intent ipc = new Intent("com.manhmoc.edgebar.IPC_ACTION");
        ipc.putExtra("act", act);
        if (act.equals("LAUNCH_APP")) {
            ipc.putExtra("launch_pkg", prefs.getString("tile_3_launch_pkg", ""));
        }
        sendBroadcast(ipc);
    }
    Tile t = getQsTile();
