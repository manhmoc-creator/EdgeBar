...
public class EdgeBarService extends AccessibilityService {
    ...
    @Override protected void onServiceConnected() {
        ...
        filter.addAction("com.manhmoc.edgebar.TEST_ANIM"); filter.addAction("com.manhmoc.edgebar.MORSE_UNLOCK_SUCCESS");
        ...
        NotificationChannel c = new NotificationChannel("eb_19_acc", "Edge Bar đang chạy nền", NotificationManager.IMPORTANCE_LOW);
        getSystemService(NotificationManager.class).createNotificationChannel(c);
        Notification n = new Notification.Builder(this, "eb_19_acc").setContentTitle("Edge Bar").setSmallIcon(android.R.drawable.ic_lock_lock).setOngoing(true).build();
        startForeground(1, n);
        createFloatingBars();
    }
    ...
    @Override public void onAccessibilityEvent(AccessibilityEvent event) {
        if(event.getEventType() == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
            ...
            if(!pName.isEmpty() && !locklist.isEmpty()) {
                for(String pkg: locklist.split(",")) if(pkg.trim().equals(pName)) { isAppLocked=true; break; }
            }
            if(isAppLocked) {
    ...
    private void createFloatingBars() {
        ...
        for(int i=0;i<4;i++){ corners[i]=new CornerView(this,i); WindowManager.LayoutParams p=new WindowManager.LayoutParams(1,1,WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY,0,PixelFormat.TRANSLUCENT); try{wm.addView(corners[i],p);}catch(Exception e){} corners[i].setOnTouchListener(new SidebarTouchListener("lock_corner_"+CORNERS[i],corners[i])); }
        updateVisibility();
    }
    private void updateVisibility() {
        ...
        for(int i=0;i<5;i++){ if(bars[i]==null) continue; boolean en = prefs.getBoolean("lock_"+BARS[i]+"_en",false); bars[i].setVisibility((en&&isLocked&&!hide)?View.VISIBLE:View.GONE); if(en&&isLocked){
            int alpha=prefs.getInt("lock_"+BARS[i]+"_alpha",50), w=prefs.getInt("lock_"+BARS[i]+"_w",300), h=prefs.getInt("lock_"+BARS[i]+"_h",60), x=prefs.getInt("lock_"+BARS[i]+"_x",0), y=prefs.getInt("lock_"+BARS[i]+"_y",0);
        ...
        for(int i=0;i<4;i++){ if(corners[i]==null) continue; boolean cornEn=prefs.getBoolean("lock_corner_"+CORNERS[i]+"_en",false); corners[i].setVisibility((cornEn&&isLocked&&!hide)?View.VISIBLE:View.GONE); if(cornEn&&isLocked){
            String ck="lock_corner_"+CORNERS[i]+"_";
            int moonAlpha=prefs.getInt("lock_corner_moon_alpha",100), strokeAlpha=prefs.getInt("lock_corner_stroke_alpha",200), hideDelay=prefs.getInt("lock_corner_hide_dur",2500);
