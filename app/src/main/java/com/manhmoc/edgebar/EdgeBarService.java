...
public class EdgeBarService extends AccessibilityService {
    ...
    private class FlashView extends View { 
        ...
        public void updateStyle() { p.setStrokeWidth(prefs.getInt("anim_thick", 12)); radius = prefs.getInt("anim_rad", 40); cTheme = prefs.getString("anim_color", "WHITE"); aStyle = prefs.getInt("anim_style", 0); if(getWidth() > 0) applyGradient(getWidth(), getHeight()); invalidate(); }
        ...
        @Override protected void onDraw(Canvas canvas) { 
            if(aStyle > 0) { 
                ...
                if (aStyle == 1) p.setPathEffect(new DashPathEffect(new float[]{perim/4f, 3*perim/4f}, phase)); // 1 tia
                else if (aStyle == 2) p.setPathEffect(new DashPathEffect(new float[]{perim/8f, 3*perim/8f}, phase)); // 2 tia đối xứng
                else if (aStyle == 3) p.setPathEffect(new DashPathEffect(new float[]{perim/12f, 3*perim/12f}, phase)); // 3 tia cách đều
    ...
    private void handleAction(String key) { 
        String action = prefs.getString(key, "NONE"); if (!action.equals("NONE")) { 
    ...
    private void updateVisibility() { 
        ...
        boolean cornInv = prefs.getBoolean("lock_corner_invis", false);
        for(int i=0; i<4; i++) { if(corners[i] == null) continue; boolean cornEn = prefs.getBoolean("lock_corner_"+CORNERS[i]+"_en", true); corners[i].setVisibility((cornEn && isLocked && !hide) ? View.VISIBLE : View.GONE); if(cornEn && isLocked) { ((CornerView)corners[i]).updateProps(prefs.getInt("lock_corner_thick", 8), cornInv ? 0 : prefs.getInt("lock_corner_alpha", 180)); WindowManager.LayoutParams p = (WindowManager.LayoutParams) corners[i].getLayoutParams(); p.gravity = C_GRAV[i]; p.x = prefs.getInt("lock_corner_w", 0); p.y = prefs.getInt("lock_corner_h", 0); wm.updateViewLayout(corners[i], p); } }
    }

    private class CornerTouchListener implements View.OnTouchListener {
        private int idx; private float sx, sy; private long st;
        public CornerTouchListener(int i) { this.idx = i; }
