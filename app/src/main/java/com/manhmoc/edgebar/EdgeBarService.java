...
public class EdgeBarService extends AccessibilityService {
    ...
    private class FlashView extends View { 
        ...
        @Override protected void onDraw(Canvas canvas) { 
            // V19.11.5.1: DÙNG ĐỘ PHÂN GIẢI THỰC CỦA MÀN HÌNH ĐỂ VẼ ANIMATION KHI FV CHƯA KỊP PHÌNH TO
    ...
    private class CornerView extends View { 
        ...
        @Override protected void onDraw(Canvas canvas) { super.onDraw(canvas); 
            ...
            boolean expandMoon = prefs.getBoolean("lock_corner_"+CORNERS[type]+"_expand", false);
            ...
            if(type==0) { 
                ...
                if(expandMoon) { moonPath.moveTo(startX, startY); moonPath.quadTo(ctrlX, ctrlY, endX, endY); moonPath.lineTo(0, 0); moonPath.lineTo(w, 0); moonPath.close(); strokePath.moveTo(startX, startY); strokePath.quadTo(ctrlX, ctrlY, endX, endY);
                } else { if(flatFactor > 0.99) { moonPath.moveTo(w-pad, h-pad); moonPath.lineTo(0, h); moonPath.lineTo(w, 0); moonPath.close(); strokePath.moveTo(0+pad, h-pad); strokePath.lineTo(w-pad, 0+pad); } 
                ...
                if(expandMoon) { moonPath.moveTo(startX, startY); moonPath.quadTo(ctrlX, ctrlY, endX, endY); moonPath.lineTo(w, 0); moonPath.lineTo(0, 0); moonPath.close(); strokePath.moveTo(startX, startY); strokePath.quadTo(ctrlX, ctrlY, endX, endY);
                } else { if(flatFactor > 0.99) { moonPath.moveTo(pad, h-pad); moonPath.lineTo(w, h); moonPath.lineTo(pad, 0); moonPath.close(); strokePath.moveTo(w-pad, h-pad); strokePath.lineTo(pad, 0+pad); } 
                ...
                if(expandMoon) { moonPath.moveTo(startX, startY); moonPath.quadTo(ctrlX, ctrlY, endX, endY); moonPath.lineTo(0, h); moonPath.lineTo(w, h); moonPath.close(); strokePath.moveTo(startX, startY); strokePath.quadTo(ctrlX, ctrlY, endX, endY);
                } else { if(flatFactor > 0.99) { moonPath.moveTo(w-pad, pad); moonPath.lineTo(0, pad); moonPath.lineTo(w, h); moonPath.close(); strokePath.moveTo(0+pad, pad); strokePath.lineTo(w-pad, h-pad); } 
                ...
                if(expandMoon) { moonPath.moveTo(startX, startY); moonPath.quadTo(ctrlX, ctrlY, endX, endY); moonPath.lineTo(w, h); moonPath.lineTo(0, h); moonPath.close(); strokePath.moveTo(startX, startY); strokePath.quadTo(ctrlX, ctrlY, endX, endY);
                } else { if(flatFactor > 0.99) { moonPath.moveTo(pad, pad); moonPath.lineTo(w, pad); moonPath.lineTo(pad, h); moonPath.close(); strokePath.moveTo(w-pad, pad); strokePath.lineTo(pad, h-pad); } 
            ...
            if(radSlider < 990 || expandMoon) canvas.drawPath(moonPath, pFill); 
    ...
    private void playAnim() {
        ...
        // V19.11.5.1: DÙNG METRICS ĐỂ TÍNH P NGAY TỨC THÌ
        float wPref = prefs.getInt("anim_w", 0); float hPref = prefs.getInt("anim_h", 0);
        android.util.DisplayMetrics metrics = new android.util.DisplayMetrics(); wm.getDefaultDisplay().getRealMetrics(metrics);
        float drawW = (wPref > 0) ? wPref : metrics.widthPixels; float drawH = (hPref > 0) ? hPref : metrics.heightPixels; 
    ...
    private void updateVisibility() { 
        ...
        for(int i=0; i<5; i++) { if(bars[i] == null) continue; boolean en = prefs.getBoolean("lock_"+BARS[i]+"_en", i < 2); bars[i].setVisibility((en && isLocked && !hide) ? View.VISIBLE : View.GONE); if(en && isLocked) { int alpha = prefs.getInt("lock_"+BARS[i]+"_alpha", 50); int w = prefs.getInt("lock_"+BARS[i]+"_w", 300); int h = prefs.getInt("lock_"+BARS[i]+"_h", 60); int x = prefs.getInt("lock_"+BARS[i]+"_x", 0); int y = prefs.getInt("lock_"+BARS[i]+"_y", 0); GradientDrawable gd = new GradientDrawable(); gd.setColor(Color.argb(alpha, 96, 125, 139)); gd.setCornerRadius(24f); bars[i].setBackground(gd); boolean isPri = prefs.getBoolean("lock_"+BARS[i]+"_pri", true); boolean isPass = prefs.getBoolean("lock_"+BARS[i]+"_pass", false); int baseFlags = WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED | WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN | WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS; if(isPass) baseFlags |= WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE; else if(isPri) baseFlags |= (WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL | WindowManager.LayoutParams.FLAG_WATCH_OUTSIDE_TOUCH); else baseFlags |= WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE; WindowManager.LayoutParams p = (WindowManager.LayoutParams) bars[i].getLayoutParams(); p.flags = baseFlags; p.width = w; p.height = h; p.x = x; p.y = y; p.gravity = GRAV[i]; wm.updateViewLayout(bars[i], p); } } 
        ...
        for(int i=0; i<4; i++) { 
            if(corners[i] == null) continue; boolean cornEn = prefs.getBoolean("lock_corner_"+CORNERS[i]+"_en", true); corners[i].setVisibility((cornEn && isLocked && !hide) ? View.VISIBLE : View.GONE); if(cornEn && isLocked) { 
                ...
                boolean inv = prefs.getBoolean("lock_corner_"+CORNERS[i]+"_inv", false);
                ((CornerView)corners[i]).updateProps(prefs.getInt("lock_corner_thick", 8), moonAlpha, strokeAlpha, isAuto, hideDelay, inv); 
                
                boolean isPri = prefs.getBoolean("lock_corner_"+CORNERS[i]+"_pri", true); boolean isPass = prefs.getBoolean("lock_corner_"+CORNERS[i]+"_pass", false);
                ...
                if(isPass) baseFlags |= WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE; else if(isPri) baseFlags |= (WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL | WindowManager.LayoutParams.FLAG_WATCH_OUTSIDE_TOUCH); else baseFlags |= WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE; 
                ...
                int gX = prefs.getInt("lock_corner_global_x", 500) - 500; int gY = prefs.getInt("lock_corner_global_y", 500) - 500; int offX = prefs.getInt("lock_corner_off_x", 0); int offY = prefs.getInt("lock_corner_off_y", 0);
                
                if(i == 0) { p.x = offX - gX; p.y = offY - gY; } else if(i == 1) { p.x = offX + gX; p.y = offY - gY; } else if(i == 2) { p.x = offX - gX; p.y = offY + gY; } else if(i == 3) { p.x = offX + gX; p.y = offY + gY; } 
