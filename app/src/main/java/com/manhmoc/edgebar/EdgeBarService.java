...
public class EdgeBarService extends AccessibilityService {
    ...
    private class CornerView extends View { 
        ...
        // V19.11.4.3: NHẬN THÊM BIẾN isSplit
        ...
        @Override protected void onDraw(Canvas canvas) { super.onDraw(canvas); 
            ...
            // V19.11.4.3: TÁCH VỎ VÀ LÕI - Không vẽ đường thẳng cho Vỏ nếu isSplit là true
            if(type==0) { // Bottom Right
                ...
                if(flatFactor > 0.99) { 
                    ...
                    if(!isSplit) { strokePath.lineTo(0, h-pad); strokePath.lineTo(0, 0); strokePath.lineTo(w-pad, 0); strokePath.close(); }
                }
                ...
                if(flatFactor > 0.99) { 
                    ...
                    strokePath.moveTo(startX, startY); strokePath.quadTo(ctrlX, ctrlY, endX, endY); 
                    if(!isSplit) { strokePath.lineTo(w, h-pad); strokePath.lineTo(w, 0); strokePath.lineTo(pad, 0); strokePath.close(); }
                }
                ...
                if(flatFactor > 0.99) { 
                    ...
                    strokePath.moveTo(startX, startY); strokePath.quadTo(ctrlX, ctrlY, endX, endY); 
                    if(!isSplit) { strokePath.lineTo(w-pad, h); strokePath.lineTo(0, h); strokePath.lineTo(0, pad); strokePath.close(); }
                }
                ...
                if(flatFactor > 0.99) { 
                    ...
                    strokePath.moveTo(startX, startY); strokePath.quadTo(ctrlX, ctrlY, endX, endY); 
                    if(!isSplit) { strokePath.lineTo(w, pad); strokePath.lineTo(w, h); strokePath.lineTo(pad, h); strokePath.close(); }
                }
            }
    ...
    private void playAnim() {
        ...
        if(style == 0) { ValueAnimator anim = ValueAnimator.ofFloat(0f, 1f, 0f); anim.setDuration(dur); anim.addUpdateListener(a -> { float val = (float)a.getAnimatedValue(); fV.setAlpha(val); }); 
            // V19.11.4.3: GIẢI QUYẾT TAPJACKING BẰNG CÁCH SET GONE SAU KHI CHẠY XONG
    ...
    private void createFloatingBars() {
        // V19.11.4.3: KHỞI TẠO VỚI TRẠNG THÁI GONE ĐỂ CHỐNG TAPJACKING TỪ ĐẦU
    ...
    private void updateVisibility() { 
        ...
        // NẾU VÀO BLACKLIST/GBOARD -> GONE NGAY LẬP TỨC
        ...
        for(int i=0; i<5; i++) { if(bars[i] == null) continue; boolean en = prefs.getBoolean("lock_"+BARS[i]+"_en", i < 2); bars[i].setVisibility((en && isLocked && !hide) ? View.VISIBLE : View.GONE); if(en && isLocked) { 
            ...
            else baseFlags |= WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE; 
        ...
        for(int i=0; i<4; i++) { 
            if(corners[i] == null) continue; boolean cornEn = prefs.getBoolean("lock_corner_"+CORNERS[i]+"_en", true); corners[i].setVisibility((cornEn && isLocked && !hide) ? View.VISIBLE : View.GONE); if(cornEn && isLocked) { 
                ...
                boolean split = prefs.getBoolean("lock_corner_"+CORNERS[i]+"_split", false); // V19.11.4.3

                ((CornerView)corners[i]).updateProps(prefs.getInt("lock_corner_thick", 8), moonAlpha, strokeAlpha, isAuto, hideDelay, inv, split); 
                
                boolean isPri = prefs.getBoolean("lock_corner_"+CORNERS[i]+"_pri", true); 
                boolean isPass = prefs.getBoolean("lock_corner_"+CORNERS[i]+"_pass", false);
                ...
                int widthPref = (i > 1) ? prefs.getInt("lock_corner_top_w", 100) : prefs.getInt("lock_corner_bot_w", 100);
                int heightPref = (i > 1) ? prefs.getInt("lock_corner_top_h", 100) : prefs.getInt("lock_corner_bot_h", 100);
