...
public class EdgeBarService extends AccessibilityService {
    ...
    private class CornerView extends View { 
        private Paint p; private int type; private String prefixKey;
        public CornerView(Context c, int type, String prefix) { super(c); this.type = type; this.prefixKey = prefix; p = new Paint(); p.setColor(Color.WHITE); p.setAlpha(200); p.setStyle(Paint.Style.STROKE); p.setAntiAlias(true); p.setStrokeCap(Paint.Cap.ROUND); } 
        public void updateProps(int thick, int alpha) { p.setStrokeWidth(thick); p.setAlpha(alpha); invalidate(); }
        @Override protected void onDraw(Canvas canvas) { super.onDraw(canvas); Path path = new Path(); float w = getWidth(), h = getHeight(), pad = p.getStrokeWidth()/2; float rad = prefs.getInt(prefixKey + "rad", 40);
    ...
    @Override protected void onServiceConnected() {
        ...
        if (Build.VERSION.SDK_INT >= 33) {
            registerReceiver(stateReceiver, filterState, Context.RECEIVER_NOT_EXPORTED);
            registerReceiver(ipcReceiver, filterIpc, Context.RECEIVER_NOT_EXPORTED);
            registerReceiver(animReceiver, filterAnim, Context.RECEIVER_NOT_EXPORTED);
        } else {
            registerReceiver(stateReceiver, filterState);
            registerReceiver(ipcReceiver, filterIpc);
            registerReceiver(animReceiver, filterAnim);
        }
    ...
    private void createFloatingBars() {
        ...
        for(int i=0; i<4; i++) { corners[i] = new CornerView(this, i, "lock_corner_"); WindowManager.LayoutParams cp = new WindowManager.LayoutParams(70, 70, WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY, WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE | WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED, PixelFormat.TRANSLUCENT); try { wm.addView(corners[i], cp); } catch(Exception e){} corners[i].setOnTouchListener(new CornerTouchListener(i)); } 
        updateVisibility();
    }
    ...
    private void updateVisibility() { 
        ...
        if (pHome || pSpy || pMorse) {
            for(View v : bars) if(v!=null) v.setVisibility(View.GONE);
            for(View v : corners) if(v!=null) v.setVisibility(View.GONE);
            if(fV != null) fV.setVisibility(View.GONE);
            return;
        }
        
        if (fV != null) fV.setVisibility(View.VISIBLE);
        boolean showLock = pLock || (isLocked && !hide);
        for(int i=0; i<5; i++) { 
            if(bars[i] == null) continue; boolean en = prefs.getBoolean("lock_"+BARS[i]+"_en", i < 2); bars[i].setVisibility((en && showLock) ? View.VISIBLE : View.GONE); 
            if(en && showLock) { int alpha = prefs.getInt("lock_"+BARS[i]+"_alpha", 50); int w = prefs.getInt("lock_"+BARS[i]+"_w", 300); int h = prefs.getInt("lock_"+BARS[i]+"_h", 60); int x = prefs.getInt("lock_"+BARS[i]+"_x", 0); int y = prefs.getInt("lock_"+BARS[i]+"_y", 0); GradientDrawable gd = new GradientDrawable(); gd.setColor(Color.argb(alpha, 96, 125, 139)); gd.setCornerRadius(24f); bars[i].setBackground(gd); WindowManager.LayoutParams p = (WindowManager.LayoutParams) bars[i].getLayoutParams(); p.width = w; p.height = h; p.x = x; p.y = y; p.gravity = GRAV[i]; if(!prefs.getBoolean("lock_"+BARS[i]+"_block", true)) p.flags |= WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE; else p.flags &= ~WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE; wm.updateViewLayout(bars[i], p); } 
        } 
        
        String cPfx = "lock_corner_";
        for(int i=0; i<4; i++) { 
            if(corners[i] == null) continue; boolean cornEn = prefs.getBoolean(cPfx+CORNERS[i]+"_en", true); boolean cornInv = prefs.getBoolean(cPfx+CORNERS[i]+"_invis", false); boolean cornBlk = prefs.getBoolean(cPfx+CORNERS[i]+"_block", true); corners[i].setVisibility((cornEn && showLock) ? View.VISIBLE : View.GONE); 
            if(cornEn && showLock) { 
                ((CornerView)corners[i]).updateProps(prefs.getInt(cPfx+"thick", 8), cornInv ? 0 : 200); 
                WindowManager.LayoutParams p = (WindowManager.LayoutParams) corners[i].getLayoutParams(); p.gravity = C_GRAV[i]; p.x = prefs.getInt(cPfx+"w", 0); p.y = prefs.getInt(cPfx+"h", 0); 
                if(!cornBlk) p.flags |= WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE; else p.flags &= ~WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE; 
