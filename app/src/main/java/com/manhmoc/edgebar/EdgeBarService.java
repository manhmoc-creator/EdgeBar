...
public class EdgeBarService extends AccessibilityService {
    ...
    private SharedPreferences.OnSharedPreferenceChangeListener prefListener = (p, k) -> { if(k != null && (k.contains("_w") || k.contains("_h") || k.contains("_x") || k.contains("_y") || k.contains("_alpha") || k.contains("_en") || k.equals("corner_en"))) updateVisibility(); };
    ...
    @Override protected void onServiceConnected() {
        ...
        String cid = "eb_v13_acc"; NotificationChannel c = new NotificationChannel(cid, "EdgeBar V13 Lockscreen", NotificationManager.IMPORTANCE_LOW); getSystemService(NotificationManager.class).createNotificationChannel(c); Notification n = new Notification.Builder(this, cid).setContentTitle("V13 Trợ Năng Màn Khoá").setSmallIcon(android.R.drawable.ic_lock_lock).setOngoing(true).build(); startForeground(1, n); createFloatingBars();
    }
    ...
    private void handleAction(String suffix) {
        ...
        if (!action.equals("NONE")) {
            if (prefs.getBoolean(key + "_anim", true)) { ObjectAnimator.ofFloat(fV, "alpha", 0f, 0.5f, 0f).setDuration(1000).start(); }
            exec(action);
        }
    }
    ...
    private void updateVisibility() {
        ...
        for(int i=0; i<5; i++) { if(bars[i] == null) continue; boolean en = prefs.getBoolean(BARS[i]+"_en", i < 2); bars[i].setVisibility((en && isLocked) ? View.VISIBLE : View.GONE); if(en && isLocked) { int alpha = prefs.getInt(BARS[i]+"_alpha", 50); int w = prefs.getInt(BARS[i]+"_w", 300); int h = prefs.getInt(BARS[i]+"_h", 60); int x = prefs.getInt(BARS[i]+"_x", 0); int y = prefs.getInt(BARS[i]+"_y", 0); GradientDrawable gd = new GradientDrawable(); gd.setColor(Color.argb(alpha, 96, 125, 139)); gd.setCornerRadius(24f); bars[i].setBackground(gd); WindowManager.LayoutParams p = (WindowManager.LayoutParams) bars[i].getLayoutParams(); p.width = w; p.height = h; p.x = x; p.y = y; p.gravity = GRAV[i]; wm.updateViewLayout(bars[i], p); } }
        boolean cornEn = prefs.getBoolean("corner_en", true);
        int cornVis = (cornEn && isLocked) ? View.VISIBLE : View.GONE;
