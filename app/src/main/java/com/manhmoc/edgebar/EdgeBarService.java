...
public class EdgeBarService extends AccessibilityService {
    ...
    private class FlashView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            float left = off;
            float top = off;
            float right = drawW - off;
            float bottom = drawH - off;
            p.setStrokeCap(Paint.Cap.ROUND);
            if (aStyle > 0) {
    ...
    private class CornerView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if (type == 0) { // BR
                ...
                sTipX = tw - sw + pad;
                sTipY = th - sh + pad;
                sCtrlX = sRootX - (1f - sRad) * (sw * 0.7f);
                sCtrlY = sRootY - (1f - sRad) * (sh * 0.7f);
                mRootX = tw;
                mRootY = th;
                mTipX = tw - mw;
                mTipY = th - mh;
                mCtrlX = mRootX - (1f - mRad) * (mw * 0.7f);
                mCtrlY = mRootY - (1f - mRad) * (mh * 0.7f);
            } else if (type == 1) { // BL
                sRootX = pad;
                sRootY = th - pad;
                sTipX = sw - pad;
