...
import android.accessibilityservice.AccessibilityService; import android.animation.ObjectAnimator; import android.app.Notification; import android.app.NotificationChannel; import android.app.NotificationManager; import android.app.KeyguardManager; import android.content.BroadcastReceiver; import android.content.Context; import android.content.Intent; import android.content.IntentFilter; import android.content.SharedPreferences; import android.graphics.Canvas; import android.graphics.Color; import android.graphics.Paint; import android.graphics.Path; import android.graphics.PixelFormat; import android.graphics.drawable.GradientDrawable; import android.hardware.camera2.CameraManager; import android.media.AudioManager; import android.provider.MediaStore; import android.view.GestureDetector; import android.view.Gravity; import android.view.MotionEvent; import android.view.View; import android.view.WindowManager; import android.view.accessibility.AccessibilityEvent;

public class EdgeBarService extends AccessibilityService {
    private WindowManager wm; private View[] bars = new View[5]; private View lLockCorner, rLockCorner; private FlashView fV; private CameraManager cm; private String cId; private boolean fOn = false, isKbd = false; private KeyguardManager km; private SharedPreferences prefs; 
    ...
    private class AssistantCurveView extends View {
        ...
        @Override protected void onDraw(Canvas canvas) { super.onDraw(canvas); Path path = new Path(); float w = getWidth(), h = getHeight(), pad = 5f; if(isLeft) { path.moveTo(pad, 0); path.quadTo(pad, h-pad, w, h-pad); } else { path.moveTo(w-pad, 0); path.quadTo(w-pad, h-pad, 0, h-pad); } canvas.drawPath(path, p); }
    }
    ...
    @Override protected void onServiceConnected() {
        ...
        String cid = "eb_v14_acc"; NotificationChannel c = new NotificationChannel(cid, "EdgeBar V14 Lock", NotificationManager.IMPORTANCE_LOW); getSystemService(NotificationManager.class).createNotificationChannel(c); Notification n = new Notification.Builder(this, cid).setContentTitle("V14 Trợ Năng Màn Khoá").setSmallIcon(android.R.drawable.ic_lock_lock).setOngoing(true).build(); startForeground(1, n); createFloatingBars();
    }

    // KBD RADAR: BẮT TÍN HIỆU BÀN PHÍM VÀ PHÁT SÓNG CHO MÀN CHÍNH
    @Override public void onAccessibilityEvent(AccessibilityEvent event) {
        if(event.getEventType() == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
            ...
            updateVisibility(); Intent i = new Intent("com.manhmoc.edgebar.KBD_STATE"); i.putExtra("isKbd", isKbd); sendBroadcast(i);
        }
    }
    ...
    private void handleAction(String suffix) {
        String key = "lock_" + suffix; String action = prefs.getString(key, "NONE");
    ...
    private void createFloatingBars() {
        ...
        lLockCorner.setOnTouchListener(lockCornerTouch); rLockCorner.setOnTouchListener(lockCornerTouch); try { wm.addView(lLockCorner, lpC); wm.addView(rLockCorner, rpC); } catch(Exception e){}
        updateVisibility();
    }

    private void updateVisibility() {
        boolean isLocked = km.isKeyguardLocked(); boolean avoidKbd = prefs.getBoolean("avoid_kbd", true); boolean hideForKbd = avoidKbd && isKbd;
        for(int i=0; i<5; i++) { if(bars[i] == null) continue; boolean en = prefs.getBoolean(BARS[i]+"_en", i < 2); bars[i].setVisibility((en && isLocked && !hideForKbd) ? View.VISIBLE : View.GONE); if(en && isLocked) { int alpha = prefs.getInt(BARS[i]+"_alpha", 50); int w = prefs.getInt(BARS[i]+"_w", 300); int h = prefs.getInt(BARS[i]+"_h", 60); int x = prefs.getInt(BARS[i]+"_x", 0); int y = prefs.getInt(BARS[i]+"_y", 0); GradientDrawable gd = new GradientDrawable(); gd.setColor(Color.argb(alpha, 96, 125, 139)); gd.setCornerRadius(24f); bars[i].setBackground(gd); WindowManager.LayoutParams p = (WindowManager.LayoutParams) bars[i].getLayoutParams(); p.width = w; p.height = h; p.x = x; p.y = y; p.gravity = GRAV[i]; wm.updateViewLayout(bars[i], p); } }
        boolean cornEn = prefs.getBoolean("lock_corner_en", true); int cornVis = (cornEn && isLocked && !hideForKbd) ? View.VISIBLE : View.GONE;
        if (lLockCorner != null) lLockCorner.setVisibility(cornVis); if (rLockCorner != null) rLockCorner.setVisibility(cornVis);
    }
