...
public class EdgeBarService extends AccessibilityService {
    ...
    // V19.1 RUNNING ANIMATION ENGINE
    private class FlashView extends View { 
        private Paint p = new Paint(); float radius = 40f; String cTheme = "WHITE"; int aStyle = 0;
        private Matrix matrix = new Matrix(); private float rotation = 0f;
        ...
        private void applyGradient(int w, int h) {
            int c1, c2; switch(cTheme) { case "NEON": c1=Color.parseColor("#FF00FF"); c2=Color.parseColor("#00FFFF"); break; case "CYBERPUNK": c1=Color.parseColor("#8A2BE2"); c2=Color.parseColor("#FFD700"); break; case "LAVA": c1=Color.parseColor("#FF4500"); c2=Color.parseColor("#FF8C00"); break; case "OCEAN": c1=Color.parseColor("#00BFFF"); c2=Color.parseColor("#1E90FF"); break; case "MATRIX": c1=Color.parseColor("#00FF00"); c2=Color.parseColor("#008000"); break; case "SUNSET": c1=Color.parseColor("#FF1493"); c2=Color.parseColor("#FF8C00"); break; default: c1=Color.WHITE; c2=Color.WHITE; break; }
            if (aStyle == 0) { p.setShader(new LinearGradient(0, 0, w, h, new int[]{c1,c2}, null, Shader.TileMode.CLAMP)); } 
            else { 
                float length = aStyle == 1 ? 0.25f : 0.5f; // 25% or 50% running light
                p.setShader(new SweepGradient(w/2f, h/2f, new int[]{Color.TRANSPARENT, c1, c2, Color.TRANSPARENT}, new float[]{0f, length-0.1f, length, length+0.01f})); 
            }
            p.setShadowLayer(8f, 0, 0, c1);
        }
        public void setRotationAngle(float angle) { this.rotation = angle; if(p.getShader() != null && aStyle > 0) { matrix.setRotate(angle, getWidth()/2f, getHeight()/2f); p.getShader().setLocalMatrix(matrix); } invalidate(); }
    ...
    // ... curve view & base logic omitted for brevity ...
    ...
    private void handleAction(String suffix) { 
        String key = "lock_" + suffix; String action = prefs.getString(key, "NONE"); if (action.equals("NONE")) { key = "both_" + suffix; action = prefs.getString(key, "NONE"); } 
        if (!action.equals("NONE")) { 
            if (prefs.getBoolean(key + "_anim", true)) { 
                int style = prefs.getInt("anim_style", 0); int dur = prefs.getInt("anim_dur", 1500);
                if(style == 0) { // FADE
                    ValueAnimator anim = ValueAnimator.ofFloat(0f, 0.5f, 0f); anim.setDuration(dur);
                    anim.addUpdateListener(a -> fV.setAlpha((float)a.getAnimatedValue())); anim.start();
                } else { // RUNNING
                    fV.setAlpha(1f); ValueAnimator anim = ValueAnimator.ofFloat(0f, 360f * 2); anim.setDuration(dur);
                    anim.addUpdateListener(a -> fV.setRotationAngle((float)a.getAnimatedValue()));
                    anim.addListener(new AnimatorListenerAdapter() { @Override public void onAnimationEnd(Animator a) { fV.setAlpha(0f); } }); anim.start();
                }
            } exec(action); 
        } 
    }
