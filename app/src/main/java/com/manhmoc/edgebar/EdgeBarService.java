...
import android.accessibilityservice.AccessibilityService; import android.animation.ObjectAnimator; import android.app.Notification; import android.app.NotificationChannel; import android.app.NotificationManager; import android.app.KeyguardManager; import android.content.Context; import android.content.Intent; import android.content.SharedPreferences; import android.graphics.Canvas; import android.graphics.Color; import android.graphics.Paint; import android.graphics.PixelFormat; import android.graphics.drawable.GradientDrawable; import android.hardware.camera2.CameraManager; import android.media.AudioManager; import android.provider.MediaStore; import android.view.GestureDetector; import android.view.Gravity; import android.view.MotionEvent; import android.view.View; import android.view.WindowManager; import android.view.accessibility.AccessibilityEvent;
...
public class EdgeBarService extends AccessibilityService {
    ...
    private final String[] BARS = {"b_l", "b_r", "t_l", "t_r", "t_c"}; private final int[] GRAV = {Gravity.BOTTOM|Gravity.LEFT, Gravity.BOTTOM|Gravity.RIGHT, Gravity.TOP|Gravity.LEFT, Gravity.TOP|Gravity.RIGHT, Gravity.TOP|Gravity.CENTER_HORIZONTAL};
    ...
    private class FlashView extends View {
        private Paint p = new Paint(); public FlashView(Context c) { super(c); p.setColor(Color.WHITE); p.setStyle(Paint.Style.STROKE); p.setStrokeWidth(4f); p.setAntiAlias(true); p.setShadowLayer(4f, 0, 0, Color.WHITE); setLayerType(LAYER_TYPE_SOFTWARE, p); }
    ...
    @Override protected void onServiceConnected() {
        ...
        prefs.registerOnSharedPreferenceChangeListener(prefListener); String cid = "eb_v10"; NotificationChannel c = new NotificationChannel(cid, "EdgeBar v10", NotificationManager.IMPORTANCE_LOW); getSystemService(NotificationManager.class).createNotificationChannel(c); Notification n = new Notification.Builder(this, cid).setContentTitle("EdgeBar 10.8.1 Trợ Năng").setSmallIcon(android.R.drawable.ic_lock_lock).setOngoing(true).build(); startForeground(1, n); createFloatingBars();
    }
    ...
    private void handleAction(String suffix) { String state = km.isKeyguardLocked() ? "lock_" : "home_"; String action = prefs.getString(state + suffix, "NONE"); if (action.equals("NONE")) action = prefs.getString("both_" + suffix, "NONE"); exec(action); }
    ...
    private void createFloatingBars() {
        ...
        // FIX VIÊN THUỐC TRÒN TUYỆT ĐỐI CHO GÓC
        GradientDrawable pill = new GradientDrawable(); 
        pill.setShape(GradientDrawable.RECTANGLE); 
        pill.setColor(Color.parseColor("#99FFFFFF")); 
        pill.setCornerRadius(100f); 
        
        WindowManager.LayoutParams hp = new WindowManager.LayoutParams(80, 12, WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY, WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE | WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED, PixelFormat.TRANSLUCENT);
        lLockCorner = new View(this); lLockCorner.setBackground(pill); rLockCorner = new View(this); rLockCorner.setBackground(pill);
        ...
        View.OnTouchListener lockCornerTouch = new View.OnTouchListener() { private float sx, sy; @Override public boolean onTouch(View v, MotionEvent e) { if(!km.isKeyguardLocked()) return false; if(e.getAction() == MotionEvent.ACTION_DOWN) { sx = e.getRawX(); sy = e.getRawY(); return true; } if(e.getAction() == MotionEvent.ACTION_UP) { float dx = e.getRawX()-sx, dy = e.getRawY()-sy; if(dy < -40 && Math.abs(dx) > 40) { ObjectAnimator.ofFloat(fV, "alpha", 0f, 0.5f, 0f).setDuration(1000).start(); handleAction(v == lLockCorner ? "l_corner" : "r_corner"); } return true; } return false; } };
