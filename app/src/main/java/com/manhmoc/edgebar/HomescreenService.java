...
public class HomescreenService extends Service {
    ...
    private class ScratchView extends View {
        ...
        @Override protected void onDraw(Canvas canvas) { super.onDraw(canvas); if(crackPoints==null) generateCracks(); paint.setAlpha(80); for(int i=0;i<crackPoints.length-2;i+=2) canvas.drawLine(crackPoints[i],crackPoints[i+1],crackPoints[i+2],crackPoints[i+3],paint); paint.setStrokeWidth(3); paint.setAlpha(120); for(int i=0;i<15;i++){ int x1=random.nextInt(getWidth()), y1=random.nextInt(getHeight()), x2=x1+random.nextInt(200)-100, y2=y1+random.nextInt(200)-100; canvas.drawLine(x1,y1,x2,y2,paint); } invalidate(); }
    }
    private class FlashView extends View {
        ...
        private void applyGradient(int w,int h){ int[] cArr; switch(cTheme){ case "NEON": cArr=new int[]{Color.parseColor("#FF00FF"),Color.parseColor("#00FFFF"),Color.parseColor("#FF00FF")}; break; case "CYBERPUNK": cArr=new int[]{Color.parseColor("#8A2BE2"),Color.parseColor("#FFD700"),Color.parseColor("#8A2BE2")}; break; case "LAVA": cArr=new int[]{Color.parseColor("#FF4500"),Color.parseColor("#FF8C00"),Color.parseColor("#FF4500")}; break; case "OCEAN": cArr=new int[]{Color.parseColor("#00BFFF"),Color.parseColor("#1E90FF"),Color.parseColor("#00BFFF")}; break; case "MATRIX": cArr=new int[]{Color.parseColor("#00FF00"),Color.parseColor("#008000"),Color.parseColor("#00FF00")}; break; case "SUNSET": cArr=new int[]{Color.parseColor("#FF1493"),Color.parseColor("#FF8C00"),Color.parseColor("#FF1493")}; break; case "GOOGLE": cArr=new int[]{Color.parseColor("#EA4335"),Color.parseColor("#FBBC05"),Color.parseColor("#34A853"),Color.parseColor("#4285F4"),Color.parseColor("#EA4335")}; break; case "AURORA": cArr=new int[]{Color.parseColor("#00E5FF"),Color.parseColor("#B388FF"),Color.parseColor("#FF4081")}; break; case "ABYSS": cArr=new int[]{Color.parseColor("#00E5FF"),Color.parseColor("#1DE9B6"),Color.parseColor("#2979FF")}; break; case "COSMIC": cArr=new int[]{Color.parseColor("#4A148C"),Color.parseColor("#E91E63"),Color.parseColor("#FFD700")}; break; case "FOREST": cArr=new int[]{Color.parseColor("#1B5E20"),Color.parseColor("#4CAF50"),Color.parseColor("#FFEB3B")}; break; case "FLAME": cArr=new int[]{Color.parseColor("#B71C1C"),Color.parseColor("#FF9800"),Color.parseColor("#FFEB3B")}; break; case "MIDNIGHT": cArr=new int[]{Color.parseColor("#1A237E"),Color.parseColor("#7B1FA2"),Color.parseColor("#03A9F4")}; break; case "TROPICAL": cArr=new int[]{Color.parseColor("#00695C"),Color.parseColor("#8BC34A"),Color.parseColor("#FF9800")}; break; case "CANDY": cArr=new int[]{Color.parseColor("#F06292"),Color.parseColor("#4DD0E1"),Color.parseColor("#FFF176")}; break; default: cArr=new int[]{Color.WHITE,Color.WHITE}; } p.setShader(new LinearGradient(0,0,w,h,cArr,null,Shader.TileMode.MIRROR)); p.setShadowLayer(15f,0,0,cArr[0]); }
        ...
        @Override protected void onDraw(Canvas canvas){
            ...
            float off=p.getStrokeWidth()/2, left=off, top=off, right=drawW-off, bottom=drawH-off;
            p.setStrokeCap(Paint.Cap.ROUND);
            if(aStyle>0){ float perim=2*(drawW+drawH), currentPhase=-perim*phaseFraction; if(aStyle==1) p.setPathEffect(new DashPathEffect(new float[]{perim/4f,3*perim/4f},currentPhase)); else if(aStyle==2) p.setPathEffect(new DashPathEffect(new float[]{perim/8f,3*perim/8f},currentPhase)); else if(aStyle==3) p.setPathEffect(new DashPathEffect(new float[]{perim/12f,3*perim/12f},currentPhase)); } else p.setPathEffect(null);
            canvas.drawRoundRect(left,top,right,bottom,radius,radius,p);
        }
    }
    private class CornerView extends View {
        private Paint pFill,pStroke; private int type; private String prefix; private Handler autoHideHandler=new Handler(); private boolean isAutoHiding=false; private int baseMoonAlpha,baseStrokeAlpha,hideDelay; private boolean isInv=false;
    ...
    @Override public void onCreate(){
        ...
        IntentFilter filter=new IntentFilter(); filter.addAction(Intent.ACTION_SCREEN_OFF); filter.addAction(Intent.ACTION_SCREEN_ON); filter.addAction(Intent.ACTION_USER_PRESENT); filter.addAction("com.manhmoc.edgebar.SYNC_STATE"); filter.addAction("com.manhmoc.edgebar.TEST_ANIM"); filter.addAction("com.manhmoc.edgebar.IPC_ACTION"); filter.addAction("com.manhmoc.edgebar.MORSE_LOCK_ENGAGE"); filter.addAction("com.manhmoc.edgebar.TOGGLE_MORSE");
        ...
        NotificationChannel c=new NotificationChannel("eb_19_home","Edge Bar Màn Chính",NotificationManager.IMPORTANCE_LOW); getSystemService(NotificationManager.class).createNotificationChannel(c); startForeground(2,new Notification.Builder(this,"eb_19_home").setContentTitle("Edge Bar Màn Chính").setSmallIcon(R.drawable.ic_launcher_fg).setOngoing(true).build());
        ...
        morseContainer=new RelativeLayout(this); morseContainer.setBackgroundColor(Color.BLACK); morseContainer.setVisibility(View.GONE); morseContainer.setOnTouchListener((v,e)->{ boolean isPreview=prefs.getBoolean("preview_morse",false); return !isPreview; });
        ...
        RelativeLayout.LayoutParams tLp=new RelativeLayout.LayoutParams(-1,-2); tLp.addRule(RelativeLayout.CENTER_IN_PARENT); morseContainer.addView(tvMorseStatus,tLp);
        scratchView=new ScratchView(this); morseContainer.addView(scratchView,new RelativeLayout.LayoutParams(-1,-1));
        WindowManager.LayoutParams bgP=new WindowManager.LayoutParams(-1,-1,WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY,WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE|WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN|WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS,PixelFormat.TRANSLUCENT); try{ wm.addView(morseContainer,bgP); }catch(Exception e){}
    ...
    private void handleMorseTap(String comp, View v){
        ...
        if(mappedKey.equals("X")){ currentMorseAttempt=""; tvMorseStatus.setText(""); }
            ...
            if(currentMorseAttempt.equals(cleanMaster)){
                ...
                Intent i=new Intent("com.manhmoc.edgebar.MORSE_UNLOCK_SUCCESS"); i.putExtra("pkg",lockedPkg); sendBroadcast(i); updateVisibility();
            }else{
                morseFailCount++; doVibrate(prefs.getInt("morse_fail_vib",500));
                if(morseFailCount==1) tvMorseStatus.setText(prefs.getString("morse_insult_1","Who are u?"));
                    ...
                    isMorseLockActive=false; new Handler().postDelayed(()->updateVisibility(),500);
                }
                currentMorseAttempt="";
            }
        }else{
            currentMorseAttempt += mappedKey; tvMorseStatus.setText(currentMorseAttempt);
            morseDotHandler.removeCallbacksAndMessages(null);
            morseDotHandler.postDelayed(()->{ StringBuilder dots=new StringBuilder(); for(int i=0;i<currentMorseAttempt.length();i++) dots.append("• "); tvMorseStatus.setText(dots.toString()); }, prefs.getInt("morse_dot_delay",500));
        }
    }
    private void updateVisibility(){
        ...
        if(isMorseLockActive||isPreviewMorse){
            ...
            for(int i=0;i<8;i++){ if(mBars[i]==null) continue; boolean en=prefs.getBoolean("morse_"+M_BARS[i]+"_en",false); mBars[i].setVisibility(en?View.VISIBLE:View.GONE); if(en){
                int alpha=prefs.getInt("morse_"+M_BARS[i]+"_alpha",50), w=prefs.getInt("morse_"+M_BARS[i]+"_w",300), h=prefs.getInt("morse_"+M_BARS[i]+"_h",60), x=prefs.getInt("morse_"+M_BARS[i]+"_x",0), y=prefs.getInt("morse_"+M_BARS[i]+"_y",0);
                GradientDrawable gd=new GradientDrawable(); gd.setColor(Color.argb(alpha,96,125,139)); gd.setCornerRadius(24f); mBars[i].setBackground(gd);
                int baseFlags=WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE|WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN|WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS|WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL|WindowManager.LayoutParams.FLAG_WATCH_OUTSIDE_TOUCH;
                WindowManager.LayoutParams p=(WindowManager.LayoutParams)mBars[i].getLayoutParams(); p.flags=baseFlags; p.width=w; p.height=h; p.x=x; p.y=y; p.gravity=M_GRAV[i]; wm.updateViewLayout(mBars[i],p);
                if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.Q && mBars[i].getVisibility()==View.VISIBLE) mBars[i].setSystemGestureExclusionRects(Collections.singletonList(new Rect(0,0,w,h)));
            }}
            for(int i=0;i<4;i++){ if(mCorners[i]==null) continue; boolean cornEn=prefs.getBoolean("morse_corner_"+CORNERS[i]+"_en",false); mCorners[i].setVisibility(cornEn?View.VISIBLE:View.GONE); if(cornEn){
                String ck="morse_corner_"+CORNERS[i]+"_";
                int moonAlpha=prefs.getInt("morse_corner_moon_alpha",100), strokeAlpha=prefs.getInt("morse_corner_stroke_alpha",200), hideDelay=prefs.getInt("morse_corner_hide_dur",2500);
                int visMode=prefs.getInt(ck+"vis_mode",0); boolean isAuto=(visMode==1), isInv=(visMode==2);
                ((CornerView)mCorners[i]).updateProps(prefs.getInt("morse_corner_thick",8), moonAlpha,strokeAlpha,isAuto,hideDelay,isInv);
                int baseFlags=WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE|WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN|WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS|WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL|WindowManager.LayoutParams.FLAG_WATCH_OUTSIDE_TOUCH;
                WindowManager.LayoutParams p=(WindowManager.LayoutParams)mCorners[i].getLayoutParams(); p.flags=baseFlags; p.gravity=C_GRAV[i];
                int wPref=prefs.getInt(ck+"w",100), hPref=prefs.getInt(ck+"h",100), mwPref=prefs.getInt(ck+"moon_w",100), mhPref=prefs.getInt(ck+"moon_h",100);
                int mxOffset=Math.abs(prefs.getInt(ck+"moon_x",1250)-1250), myOffset=Math.abs(prefs.getInt(ck+"moon_y",1250)-1250);
                ...
                if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.Q && mCorners[i].getVisibility()==View.VISIBLE) mCorners[i].setSystemGestureExclusionRects(Collections.singletonList(new Rect(0,0,p.width,p.height)));
            }}
        }else{
            ...
            for(int i=0;i<5;i++){ if(bars[i]==null) continue; boolean en=prefs.getBoolean("home_"+BARS[i]+"_en",false); bars[i].setVisibility((en&&isUnlocked&&!hideNormal)?View.VISIBLE:View.GONE); if(en&&isUnlocked){
                int alpha=isPreviewLock?0:prefs.getInt("home_"+BARS[i]+"_alpha",50), w=prefs.getInt("home_"+BARS[i]+"_w",300), h=prefs.getInt("home_"+BARS[i]+"_h",60), x=prefs.getInt("home_"+BARS[i]+"_x",0), y=prefs.getInt("home_"+BARS[i]+"_y",0);
                ...
                if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.Q && bars[i].getVisibility()==View.VISIBLE && priMode==0) bars[i].setSystemGestureExclusionRects(Collections.singletonList(new Rect(0,0,w,h)));
            }}
            for(int i=0;i<4;i++){ if(corners[i]==null) continue; boolean cornEn=prefs.getBoolean("home_corner_"+CORNERS[i]+"_en",false); corners[i].setVisibility((cornEn&&isUnlocked&&!hideNormal)?View.VISIBLE:View.GONE); if(cornEn&&isUnlocked){
                ...
                int moonAlpha=isPreviewLock?0:prefs.getInt("home_corner_moon_alpha",100), strokeAlpha=isPreviewLock?0:prefs.getInt("home_corner_stroke_alpha",200), hideDelay=prefs.getInt("home_corner_hide_dur",2500);
                int visMode=prefs.getInt(ck+"vis_mode",0); boolean isAuto=(visMode==1), isInv=(visMode==2);
                ...
                if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.Q && corners[i].getVisibility()==View.VISIBLE && priMode==0) corners[i].setSystemGestureExclusionRects(Collections.singletonList(new Rect(0,0,p.width,p.height)));
            }}
        }
    }
    private void playAnim(){
        ...
        fV.post(()->{
            int style=prefs.getInt("anim_style",0), dur=prefs.getInt("anim_dur",1500); ValueAnimator anim;
            ...
            anim.setDuration(dur); anim.addListener(new AnimatorListenerAdapter(){@Override public void onAnimationEnd(Animator a){ fV.setAlpha(0f); fV.setVisibility(View.GONE); fp.width=0; fp.height=0; wm.updateViewLayout(fV,fp); }}); anim.start();
        });
    }
    private void handleAction(String key){
        String action=prefs.getString(key,"NONE"); boolean isOn=prefs.getBoolean(key+"_on",true);
        ...
        try{
            switch(a){
                case "MACRO_1": case "MACRO_2": case "MACRO_3": case "MACRO_4": case "MACRO_5":
                    Intent iM=new Intent("com.manhmoc.edgebar.TOGGLE_MACRO"); iM.putExtra("services",prefs.getString(a.toLowerCase()+"_svcs","")); sendBroadcast(iM); break;
                ...
                case "CAMERA": startActivity(new Intent(MediaStore.INTENT_ACTION_STILL_IMAGE_CAMERA_SECURE).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)); break;
    ...
    private void fireIntent(String idx){
        try{
            String act=prefs.getString("i"+idx+"_act",""), pkg=prefs.getString("i"+idx+"_pkg","");
            Intent i;
            ...
            if(i==null) return;
            if(prefs.getBoolean("i"+idx+"_br",true)&&!act.isEmpty()) sendBroadcast(i);
    ...
    private class SidebarTouchListener implements View.OnTouchListener{
        ...
        public SidebarTouchListener(String keyBase, View v){ this.prefKeyBase=keyBase; this.myView=v; this.gd=new GestureDetector(HomescreenService.this,new GestureDetector.SimpleOnGestureListener(){
