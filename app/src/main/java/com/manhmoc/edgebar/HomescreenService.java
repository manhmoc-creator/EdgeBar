...
import android.animation.ValueAnimator; import android.animation.AnimatorListenerAdapter; import android.animation.Animator;
import android.app.Notification; import android.app.NotificationChannel; import android.app.NotificationManager;
import android.app.Service; import android.app.KeyguardManager;
...
import android.graphics.*; import android.graphics.drawable.GradientDrawable;
import android.hardware.camera2.CameraManager;
import android.media.AudioManager;
import android.os.Build; import android.os.Handler; import android.os.VibrationEffect; import android.os.Vibrator; import android.os.IBinder;
...
public class HomescreenService extends Service {
    ...
    private RelativeLayout morseContainer; private TextView tvMorseStatus; private View scratchView;
    private View[] mBars = new View[8]; private View[] mCorners = new View[4];
    private FlashView fV; private CameraManager cm; private String cId; private boolean fOn=false, isKbd=false, isBl=false;
    private SharedPreferences prefs; private KeyguardManager km; private Vibrator vibrator;
    private boolean isMorseLockActive=false; private String currentMorseAttempt=""; private int morseFailCount=0;
    private String lockedPkg=""; private Handler morseDotHandler=new Handler();
    ...
    private BroadcastReceiver syncReceiver = new BroadcastReceiver() {
        @Override public void onReceive(Context c, Intent i) {
            if(i.getAction().equals("com.manhmoc.edgebar.SYNC_STATE")){
                isKbd=i.getBooleanExtra("isKbd",false); isBl=i.getBooleanExtra("isBl",false); updateVisibility();
                ...
                isMorseLockActive=true; lockedPkg=i.getStringExtra("pkg"); morseFailCount=0;
                currentMorseAttempt=""; tvMorseStatus.setText(""); updateVisibility();
            } else if(i.getAction().equals("com.manhmoc.edgebar.TOGGLE_MORSE")){
                boolean isM=prefs.getBoolean("morse_mode_en",false);
                prefs.edit().putBoolean("morse_mode_en",!isM).apply(); updateVisibility();
            }
        }
    };
    private String mapComponentToNumber(String comp){
        ...
        switch(key){
            ...
            case "m_mid_b": return "7"; case "m_b_c": return "8"; case "tl": return "9";
            case "tr": return "0"; default: return "*";
        }
    }
    private class ScratchView extends View {
        ...
        public ScratchView(Context context){ super(context);
            paint.setColor(Color.WHITE); paint.setStrokeWidth(6); paint.setStyle(Paint.Style.STROKE);
            paint.setAntiAlias(true); generateCracks();
        }
        private void generateCracks(){ crackPoints=new int[20];
            for(int i=0;i<20;i+=2){ crackPoints[i]=random.nextInt(2000); crackPoints[i+1]=random.nextInt(3000); }
        }
        @Override protected void onDraw(Canvas canvas){
            ...
            paint.setStrokeWidth(3); paint.setAlpha(120);
            for(int i=0;i<15;i++){
                int x1=random.nextInt(getWidth()), y1=random.nextInt(getHeight());
                int x2=x1+random.nextInt(200)-100, y2=y1+random.nextInt(200)-100;
    ...
    private class FlashView extends View {
        ...
        public FlashView(Context c){ super(c); p.setStyle(Paint.Style.STROKE); p.setStrokeCap(Paint.Cap.ROUND);
            p.setStrokeJoin(Paint.Join.ROUND); p.setAntiAlias(true); setLayerType(LAYER_TYPE_SOFTWARE,p); updateStyle(); }
        public void updateStyle(){ p.setAlpha(prefs.getInt("anim_alpha",255)); p.setStrokeWidth(prefs.getInt("anim_thick",12));
            radius=prefs.getInt("anim_rad",40); cTheme=prefs.getString("anim_color","WHITE"); aStyle=prefs.getInt("anim_style",0);
            if(getWidth()>0) applyGradient(getWidth(),getHeight()); invalidate(); }
        ...
        @Override protected void onDraw(Canvas canvas){
            ...
            if(aStyle>0){
                float perim=2*(drawW+drawH), currentPhase=-perim*phaseFraction;
    ...
    private class CornerView extends View {
        private Paint pFill,pStroke; private int type; private String prefix; private Handler autoHideHandler=new Handler();
        private boolean isAutoHiding=false; private int baseMoonAlpha,baseStrokeAlpha,hideDelay; private boolean isInv=false;
        public CornerView(Context c,int type,String prefix){ super(c); this.type=type; this.prefix=prefix;
    ...
    @Override public void onCreate(){
        ...
        for(int i=0;i<5;i++){ bars[i]=new View(this);
        ...
        for(int i=0;i<4;i++){ corners[i]=new CornerView(this,i,"home_");
            WindowManager.LayoutParams p=new WindowManager.LayoutParams(1,1,WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY,0,PixelFormat.TRANSLUCENT);
        ...
        morseContainer=new RelativeLayout(this); morseContainer.setBackgroundColor(Color.BLACK);
        ...
        tvMorseStatus=new TextView(this); tvMorseStatus.setTextColor(Color.WHITE); tvMorseStatus.setTextSize(30);
        ...
        for(int i=0;i<8;i++){ mBars[i]=new View(this);
            WindowManager.LayoutParams p=new WindowManager.LayoutParams(1,1,WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY,0,PixelFormat.TRANSLUCENT);
    ...
    private void handleMorseTap(String comp, View v){
        ...
        if(num.equals("*")){
        ...
        } else if(num.equals("0") && comp.contains("tr")){ // Save/Submit
            ...
            if(currentMorseAttempt.equals(masterPass)){
                ...
                Intent i=new Intent("com.manhmoc.edgebar.MORSE_UNLOCK_SUCCESS"); i.putExtra("pkg",lockedPkg);
                sendBroadcast(i); updateVisibility();
            } else {
                morseFailCount++;
                int failVib=prefs.getInt("morse_fail_vib",500); doVibrate(failVib);
                if(morseFailCount==1) tvMorseStatus.setText(prefs.getString("morse_insult_1","Who are u?"));
                    ...
                    Intent kick=new Intent("com.manhmoc.edgebar.IPC_ACTION"); kick.putExtra("act","HOME");
                    sendBroadcast(kick); isMorseLockActive=false;
                    new Handler().postDelayed(()->updateVisibility(),500);
                }
                currentMorseAttempt="";
            }
        } else {
            currentMorseAttempt+=num; tvMorseStatus.setText(currentMorseAttempt);
    ...
    private void updateVisibility(){
        boolean isUnlocked=!km.isKeyguardLocked(); boolean avoidKbd=prefs.getBoolean("avoid_kbd",true);
        ...
        if(isMorseLockActive || isPreviewMorse){
            ...
            for(int i=0;i<8;i++){ if(mBars[i]==null) continue;
                ...
                if(en){
                    ...
                    int w=prefs.getInt("morse_"+M_BARS[i]+"_w",300), h=prefs.getInt("morse_"+M_BARS[i]+"_h",60);
                    int x=prefs.getInt("morse_"+M_BARS[i]+"_x",0), y=prefs.getInt("morse_"+M_BARS[i]+"_y",0);
                    GradientDrawable gd=new GradientDrawable(); gd.setColor(Color.argb(alpha,96,125,139)); gd.setCornerRadius(24f);
                    mBars[i].setBackground(gd);
                    int baseFlags=WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE | WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN |
                    ...
                    p.flags=baseFlags; p.width=w; p.height=h; p.x=x; p.y=y; p.gravity=M_GRAV[i];
                    wm.updateViewLayout(mBars[i],p);
                    if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.Q && mBars[i].getVisibility()==View.VISIBLE){
                        Rect rect=new Rect(0,0,w,h); mBars[i].setSystemGestureExclusionRects(Collections.singletonList(rect));
                    }
                }
            }
            for(int i=0;i<4;i++){ if(mCorners[i]==null) continue;
                ...
                if(cornEn){
                    ...
                    if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.Q && mCorners[i].getVisibility()==View.VISIBLE){
                        Rect rect=new Rect(0,0,p.width,p.height); mCorners[i].setSystemGestureExclusionRects(Collections.singletonList(rect));
                    }
                }
            }
        } else {
            ...
            for(int i=0;i<5;i++){ if(bars[i]==null) continue;
                ...
                if(en && isUnlocked){
                    ...
                    int w=prefs.getInt("home_"+BARS[i]+"_w",300), h=prefs.getInt("home_"+BARS[i]+"_h",60);
                    int x=prefs.getInt("home_"+BARS[i]+"_x",0), y=prefs.getInt("home_"+BARS[i]+"_y",0);
                    GradientDrawable gd=new GradientDrawable(); gd.setColor(Color.argb(alpha,96,125,139)); gd.setCornerRadius(24f);
                    ...
                    if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.Q && bars[i].getVisibility()==View.VISIBLE && priMode==0){
                        Rect rect=new Rect(0,0,w,h); bars[i].setSystemGestureExclusionRects(Collections.singletonList(rect));
                    }
                }
            }
            for(int i=0;i<4;i++){ if(corners[i]==null) continue;
                ...
                if(cornEn && isUnlocked){
                    ...
                    if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.Q && corners[i].getVisibility()==View.VISIBLE && priMode==0){
                        Rect rect=new Rect(0,0,p.width,p.height); corners[i].setSystemGestureExclusionRects(Collections.singletonList(rect));
                    }
                }
            }
        }
        ...
        try{
            switch(a){
                case "MACRO_1": case "MACRO_2": case "MACRO_3": case "MACRO_4": case "MACRO_5":
                    Intent iM=new Intent("com.manhmoc.edgebar.TOGGLE_MACRO"); iM.putExtra("services",prefs.getString(a.toLowerCase()+"_svcs",""));
                    sendBroadcast(iM); break;
                case "TOGGLE_MORSE": Intent m=new Intent("com.manhmoc.edgebar.TOGGLE_MORSE"); sendBroadcast(m); break;
                case "YTDL_DOWNLOAD": try{
                ...
                case "BACK": case "HOME": case "RECENTS": case "SCREEN_OFF": case "POWER_DIALOG": case "SCREENSHOT": case "NOTIFICATIONS":
                case "TOGGLE_OVERLAY": case "TOGGLE_ACC":
    ...
    private class SidebarTouchListener implements View.OnTouchListener {
        ...
        public SidebarTouchListener(String keyBase, View v){ this.prefKeyBase=keyBase; this.myView=v;
            gd=new GestureDetector(HomescreenService.this, new GestureDetector.SimpleOnGestureListener(){
