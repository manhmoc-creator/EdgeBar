...
public class HomescreenService extends Service {
    ...
    private BroadcastReceiver syncReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context c, Intent i) {
            ...
            if ("com.manhmoc.edgebar.SYNC_STATE".equals(action)) {
                isKbd = i.getBooleanExtra("isKbd", false);
                isBl = i.getBooleanExtra("isBl", false);
                updateVisibility();
            } else if ("com.manhmoc.edgebar.TEST_ANIM".equals(action)) {
                playAnim();
            } else if ("com.manhmoc.edgebar.MORSE_LOCK_ENGAGE".equals(action)) {
    ...
    public void onCreate() {
        ...
        filter.addAction("com.manhmoc.edgebar.START_MORSE_RECORD");
        ...
        Notification n = new Notification.Builder(this, cid).setContentTitle("Edge Bar Màn Chính").setSmallIcon(android.R.drawable.ic_menu_crop).setOngoing(true).build();
        startForeground(2, n);
    ...
    private void handleMorseTap(String comp, View v) {
        ...
        boolean isRecording = prefs.getBoolean("morse_recording", false);
        if (mapped.equals("X")) {
            currentMorseAttempt = "";
            tvMorseStatus.setText(isRecording ? "Cleared" : "");
        } else if (mapped.equals(">")) {
            if (isRecording) {
                prefs.edit().putString("morse_password", currentMorseAttempt).putBoolean("morse_recording", false).apply();
                isMorseLockActive = false;
                tvMorseStatus.setText("Saved!");
                ...
                if (currentMorseAttempt.equals(savedPass)) {
                    ...
                    i.putExtra("pkg", lockedPkg); sendBroadcast(i);
                    updateVisibility();
                } else {
                    morseFailCount++;
                    ...
                    if (morseFailCount == 1) tvMorseStatus.setText(prefs.getString("morse_insult_1", "Who are u?"));
                        ...
                        kick.putExtra("act", "HOME"); sendBroadcast(kick);
                        isMorseLockActive = false;
            ...
            currentMorseAttempt += mapped;
            if (isRecording) tvMorseStatus.setText("Seq: " + currentMorseAttempt);
            else {
    ...
    private void playAnim() {
        ...
        fV.post(() -> {
            ...
            anim.addListener(new AnimatorListenerAdapter() {
                @Override public void onAnimationEnd(Animator a) { fV.setAlpha(0f); fV.setVisibility(View.GONE); fp.width=0; fp.height=0; wm.updateViewLayout(fV, fp); }
            });
            anim.start();
        });
    }
    ...
    private class SidebarTouchListener implements View.OnTouchListener {
        ...
        public SidebarTouchListener(String keyBase, View v) {
            this.prefKeyBase = keyBase; this.myView = v;
