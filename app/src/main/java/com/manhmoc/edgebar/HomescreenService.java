...
public class HomescreenService extends Service {
    ...
    private class CornerView extends View { 
        ...
        @Override protected void onDraw(Canvas canvas) { super.onDraw(canvas); 
            Path path = new Path(); Path hitBox = new Path(); float w = getWidth(), h = getHeight(), pad = pStroke.getStrokeWidth()/2; float rad = prefs.getInt("home_corner_rad", 40);
    ...
    private void updateVisibility() { 
        ...
        for(int i=0; i<4; i++) { if(corners[i] == null) continue; boolean cornEn = prefs.getBoolean("home_corner_"+CORNERS[i]+"_en", true); boolean cornInv = prefs.getBoolean("home_corner_"+CORNERS[i]+"_invis", false); boolean cornBlk = prefs.getBoolean("home_corner_"+CORNERS[i]+"_block", true); corners[i].setVisibility((cornEn && isUnlocked && !hide) ? View.VISIBLE : View.GONE); if(cornEn && isUnlocked) { int alpha = cornInv ? (cornBlk ? 1 : 0) : 200; ((CornerView)corners[i]).updateProps(prefs.getInt("home_corner_thick", 8), alpha, cornInv); WindowManager.LayoutParams p = (WindowManager.LayoutParams) corners[i].getLayoutParams(); if(p==null) continue; p.gravity = C_GRAV[i]; p.x = prefs.getInt("home_corner_w", 0); p.y = prefs.getInt("home_corner_h", 0); if(!cornBlk) p.flags |= WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE; else p.flags &= ~WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE; try { wm.updateViewLayout(corners[i], p); } catch(Exception e){} } }
    }
