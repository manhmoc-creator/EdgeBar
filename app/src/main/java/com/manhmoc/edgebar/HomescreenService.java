...
public class HomescreenService extends Service {
    ...
    public void onCreate() {
        ...
        // QUAN TRỌNG: preview_morse == true => không chặn touch (để setup), false => chặn
    ...
    private void handleMorseTap(String comp, View v) {
        ...
        if (mappedKey.equals("X")) {
            ...
            if (currentMorseAttempt.equals(cleanMaster)) {
                ...
                doVibrate(failVib);
                String insult;
                if (morseFailCount == 1) insult = prefs.getString("morse_insult_1", "Who are u?");
                else if (morseFailCount == 2) insult = prefs.getString("morse_insult_2", "What are u doing?");
                else insult = prefs.getString("morse_insult_3", "Get out!");
