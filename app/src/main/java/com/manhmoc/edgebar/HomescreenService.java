...
public class HomescreenService extends Service {
    ...
    private String mapComponentToNumber(String comp) {
        // Simple mapping for Morse (can be customized)
    ...
    private class FlashView extends View {
        ...
        private void applyGradient(int w, int h) { /* same as EdgeBarService */ }
        public void setPhase(float fraction) { this.phaseFraction = fraction; invalidate(); }
        @Override protected void onDraw(Canvas canvas) { /* same as EdgeBarService */ }
    }
    ...
    private class CornerView extends View {
        ...
        @Override protected void onDraw(Canvas canvas) { /* similar to EdgeBarService but with prefix */ }
    }
    ...
    @Override public void onCreate() {
        ...
        morseContainer = new RelativeLayout(this); morseContainer.setBackgroundColor(Color.BLACK); morseContainer.setVisibility(View.GONE); morseContainer.setOnTouchListener((v, e) -> true);
        tvMorseStatus = new TextView(this); tvMorseStatus.setTextColor(Color.WHITE); tvMorseStatus.setTextSize(30); tvMorseStatus.setGravity(Gravity.CENTER); RelativeLayout.LayoutParams tLp = new RelativeLayout.LayoutParams(-1,-2); tLp.addRule(RelativeLayout.CENTER_IN_PARENT); morseContainer.addView(tvMorseStatus, tLp);
    ...
    private void handleMorseTap(String comp, View v) {
        ...
        String mapped = mapComponentToNumber(comp);
        if (mapped.equals("X")) { currentMorseAttempt = ""; tvMorseStatus.setText(""); }
        else if (mapped.equals(">")) {
            String master = prefs.getString("morse_master_pass", "");
            if (currentMorseAttempt.isEmpty() || master.isEmpty()) return;
            if (currentMorseAttempt.equals(master)) {
                ...
                morseFailCount++; int failVib = prefs.getInt("morse_fail_vib", 500); doVibrate(failVib);
                if (morseFailCount == 1) tvMorseStatus.setText(prefs.getString("morse_insult_1", "Who?"));
                else if (morseFailCount == 2) tvMorseStatus.setText(prefs.getString("morse_insult_2", "What?"));
                else { tvMorseStatus.setText(prefs.getString("morse_insult_3", "Get out!")); Intent kick = new Intent("com.manhmoc.edgebar.IPC_ACTION"); kick.putExtra("act", "HOME"); sendBroadcast(kick); isMorseLockActive = false; new Handler().postDelayed(() -> updateVisibility(), 500); }
                currentMorseAttempt = "";
            }
        } else { currentMorseAttempt += mapped; tvMorseStatus.setText(currentMorseAttempt); morseDotHandler.removeCallbacksAndMessages(null); int dotDelay = prefs.getInt("morse_dot_delay", 500); morseDotHandler.postDelayed(() -> { StringBuilder dots = new StringBuilder(); for (int i=0;i<currentMorseAttempt.length();i++) dots.append("• "); tvMorseStatus.setText(dots.toString()); }, dotDelay); }
    }
