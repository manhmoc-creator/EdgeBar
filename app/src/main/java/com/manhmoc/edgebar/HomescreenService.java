...
public class HomescreenService extends Service {
    ...
    private BroadcastReceiver syncReceiver = new BroadcastReceiver() { @Override public void onReceive(Context c, Intent i) { 
        if("com.manhmoc.edgebar.SYNC_STATE".equals(i.getAction())) { isKbd = i.getBooleanExtra("isKbd", false); isBl = i.getBooleanExtra("isBl", false); } 
        updateVisibility(); 
    }};
    ...
    @Override public void onCreate() {
        ...
        if (Build.VERSION.SDK_INT >= 33) {
            registerReceiver(syncReceiver, filterState, Context.RECEIVER_NOT_EXPORTED);
            registerReceiver(animReceiver, filterAnim, Context.RECEIVER_NOT_EXPORTED);
        } else {
            registerReceiver(syncReceiver, filterState);
        ...
        Notification n = new Notification.Builder(this, cid).setContentTitle("Edge Bar đang chạy nền (Home)").setSmallIcon(android.R.drawable.ic_menu_crop).build(); 
        startForeground(2, n);
