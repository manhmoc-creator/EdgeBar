...
public class HomescreenService extends Service {
    ...
    private class FlashView extends View { 
        ...
        @Override protected void onDraw(Canvas canvas) { 
            ...
            float off = p.getStrokeWidth()/2; float left = off; float top = off; float right = drawW - off; float bottom = drawH - off;
    ...
    private class CornerView extends View { 
        ...
        @Override protected void onDraw(Canvas canvas) { 
            ...
            String ck = "home_corner_" + CORNERS[type] + "_";
    ...
    @Override public void onCreate() {
        ...
        prefs.registerOnSharedPreferenceChangeListener(prefListener); IntentFilter filter = new IntentFilter(); filter.addAction(Intent.ACTION_SCREEN_OFF); filter.addAction(Intent.ACTION_SCREEN_ON); filter.addAction(Intent.ACTION_USER_PRESENT); filter.addAction("com.manhmoc.edgebar.SYNC_STATE"); filter.addAction("com.manhmoc.edgebar.TEST_ANIM"); filter.addAction("com.manhmoc.edgebar.IPC_ACTION");
    ...
    private void handleAction(String key) { String action = prefs.getString(key, "NONE"); if (!action.equals("NONE")) { if (prefs.getBoolean(key + "_vib", true)) doVibrate(prefs.getInt("vib_dur", 30)); if (prefs.getBoolean(key + "_anim", true)) playAnim(); String[] acts = action.split(","); for(String a : acts) exec(a.trim()); } }
    ...
    private void exec(String a) { if (a == null || a.equals("NONE")) return; try { 

        switch(a) { 

            case "BACK": case "HOME": case "RECENTS": case "SCREEN_OFF": case "POWER_DIALOG": case "SCREENSHOT": case "NOTIFICATIONS": Intent ipc = new Intent("com.manhmoc.edgebar.IPC_ACTION"); ipc.putExtra("act", a); sendBroadcast(ipc); break;

            case "FLASH": fOn = !fOn; cm.setTorchMode(cId, fOn); break; case "CAMERA": Intent c = new Intent(MediaStore.INTENT_ACTION_STILL_IMAGE_CAMERA_SECURE); c.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK); startActivity(c); break; case "VOLUME": ((AudioManager)getSystemService(AUDIO_SERVICE)).adjustStreamVolume(AudioManager.STREAM_MUSIC, AudioManager.ADJUST_SAME, AudioManager.FLAG_SHOW_UI); break; case "TOGGLE_ACC": Intent iAcc = new Intent("com.manhmoc.edgebar.TOGGLE_LOCK"); sendBroadcast(iAcc); break; case "TOGGLE_OVERLAY": Intent iOv = new Intent("com.manhmoc.edgebar.TOGGLE_HOME"); sendBroadcast(iOv); break; case "YTDL_DOWNLOAD": try{android.content.ClipboardManager cb=(android.content.ClipboardManager)getSystemService(Context.CLIPBOARD_SERVICE); if(cb.hasPrimaryClip()&&cb.getPrimaryClip().getItemCount()>0){CharSequence txt=cb.getPrimaryClip().getItemAt(0).getText(); if(txt!=null&&(txt.toString().startsWith("http"))){Intent y=new Intent(Intent.ACTION_SEND); y.setType("text/plain"); y.putExtra(Intent.EXTRA_TEXT,txt.toString()); y.setPackage("com.deniscerri.ytdlnis"); y.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK); startActivity(y);}}}catch(Exception e){} break;

            default: if(a.startsWith("INTENT_")) fireIntent(a.split("_")[1]); else if(a.startsWith("MACRO_")) { Intent iM = new Intent("com.manhmoc.edgebar.TOGGLE_MACRO"); iM.putExtra("services", prefs.getString(a.toLowerCase()+"_svcs", "")); sendBroadcast(iM); } break;
