...
public class HomescreenService extends Service {
    ...
    private void updateVisibility() { 
        ...
        for(int i=0; i<4; i++) { if(corners[i] == null) continue; boolean cEn = prefs.getBoolean("home_corner_"+CORNERS[i]+"_en", true); boolean cInv = prefs.getBoolean("home_corner_"+CORNERS[i]+"_invis", false); corners[i].setVisibility((cEn && isUnlocked && !hide) ? View.VISIBLE : View.GONE); if(cEn && isUnlocked) { ((CornerView)corners[i]).updateProps(prefs.getInt("home_corner_thick", 8), cInv ? 0 : 200); WindowManager.LayoutParams p = (WindowManager.LayoutParams) corners[i].getLayoutParams(); p.gravity = C_GRAV[i]; p.x = prefs.getInt("home_corner_w", 0); p.y = prefs.getInt("home_corner_h", 0); if(!prefs.getBoolean("home_corner_"+CORNERS[i]+"_block", true)) p.flags |= WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE; else p.flags &= ~WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE; wm.updateViewLayout(corners[i], p); } }
    }
