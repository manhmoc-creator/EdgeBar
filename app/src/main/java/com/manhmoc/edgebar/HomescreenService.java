...
public class HomescreenService extends Service {
    ...
    private void updateVisibility() { 
        ...
        boolean cornEn = prefs.getBoolean("home_corner_en", true); boolean cornInv = prefs.getBoolean("home_corner_invis", false);
        for(int i=0; i<4; i++) { if(corners[i] == null) continue; corners[i].setVisibility((cornEn && isUnlocked && !hide) ? View.VISIBLE : View.GONE); if(cornEn && isUnlocked) { ((CornerView)corners[i]).updateProps(prefs.getInt("home_corner_thick", 8), cornInv ? 0 : 200); WindowManager.LayoutParams p = (WindowManager.LayoutParams) corners[i].getLayoutParams(); p.gravity = C_GRAV[i]; p.x = prefs.getInt("home_corner_w", 0); p.y = prefs.getInt("home_corner_h", 0); if(!prefs.getBoolean("home_corner_block", true)) p.flags |= WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE; else p.flags &= ~WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE; wm.updateViewLayout(corners[i], p); } }
    }
