...
public class RecorderService extends Service {
    ...
    @Override public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        try {
            ...
            Toast.makeText(this, "🔴 Đang ghi âm ngầm...\nLưu tại: Music/EdgeBar", Toast.LENGTH_SHORT).show(); sendBroadcast(new Intent("com.manhmoc.edgebar.START_BREATH"));
        } catch(Exception e) { Toast.makeText(this, "Lỗi Mic hoặc Quyền!", Toast.LENGTH_SHORT).show(); stopSelf(); } return START_STICKY;
    }
    @Override public void onDestroy() { super.onDestroy(); try{ unregisterReceiver(stopCmd); } catch(Exception e){} try { if(rec != null && isRunning) { rec.stop(); rec.release(); isRunning = false; Toast.makeText(this, "✅ Đã lưu: " + path, Toast.LENGTH_LONG).show(); sendBroadcast(new Intent("com.manhmoc.edgebar.STOP_BREATH")); } } catch(Exception e){} }
