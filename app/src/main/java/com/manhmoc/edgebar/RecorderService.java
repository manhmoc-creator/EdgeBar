...
public class RecorderService extends Service {
    ...
    private class BreathView extends View {
        ...
        @Override protected void onDraw(Canvas canvas) {
            ...
            if (shape == 1) { canvas.drawCircle(cx, cy, dotSize, pDraw); canvas.drawText(timeStr, cx + dotSize + 15, cy + 12, pText); } 
    ...
    @Override public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent != null && "TEST_BREATH".equals(intent.getAction())) { isTestMode = true; showBreathingEdge(); return START_NOT_STICKY; }
        if (intent != null && "STOP_TEST".equals(intent.getAction())) { stopRecording(); stopSelf(); return START_NOT_STICKY; }
        
        if (isRecording) { stopRecording(); stopSelf(); } else {
            ...
            new CountDownTimer(3000, 1000) { public void onTick(long millis) { Toast.makeText(RecorderService.this, "🎙 " + (millis / 1000 + 1) + "...", Toast.LENGTH_SHORT).show(); } public void onFinish() { startRecording(); } }.start();
        } return START_STICKY;
    }
    ...
    private void startRecording() {
        ...
        try {
            ...
            try{((android.os.Vibrator) getSystemService(Context.VIBRATOR_SERVICE)).vibrate(android.os.VibrationEffect.createOneShot(80, android.os.VibrationEffect.DEFAULT_AMPLITUDE));}catch(Exception e){}
            showBreathingEdge();
    ...
    private void showBreathingEdge() {
        ...
        timerHandler.post(new Runnable() {
            @Override public void run() {
                ...
                long e = isTestMode ? 0 : System.currentTimeMillis() - startTime; breathView.setTime(isTestMode ? "TEST" : String.format("%02d:%02d", (e/1000)/60, (e/1000)%60));
    ...
    private void stopRecording() {
        ...
        isRecording = false; timerHandler.removeCallbacksAndMessages(null);
        if (breathAnim != null) breathAnim.cancel();
        if (wm != null && breathView != null) { try{wm.removeView(breathView);}catch(Exception e){} breathView = null; }
    ...
    @Override public void onDestroy() { if(isRecording || isTestMode) stopRecording(); super.onDestroy(); }
