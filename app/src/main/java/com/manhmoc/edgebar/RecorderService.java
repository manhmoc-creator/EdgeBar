...
public class RecorderService extends Service {
    ...
    private SharedPreferences prefs; private long startTime = 0; private Handler timerHandler = new Handler();
    ...
    private class BreathView extends View {
        ...
        public BreathView(Context c) { 
            ...
            shape = prefs.getInt("breath_shape", 0);
            w = prefs.getInt("breath_w", 0); h = prefs.getInt("breath_h", 0);
            thick = prefs.getInt("breath_thick", 12); dotSize = prefs.getInt("breath_dot_size", 40);
            pDraw.setStrokeWidth(thick);
            
            String cTheme = prefs.getString("anim_color", "WHITE");
            color = Color.WHITE; for(int i=0; i<COLOR_KEYS.length; i++) if(COLOR_KEYS[i].equals(cTheme)) color = Color.parseColor(COLOR_VALS[i]);
        ...
        @Override protected void onDraw(Canvas canvas) {
            ...
            if (shape == 1) { // Dot
                ...
                canvas.drawText(timeStr, cx + dotSize + 15, cy + 12, pText);
            } else { // Edge
                ...
                canvas.drawText(timeStr, cx - 40, top + drawH + 40, pText);
            }
        }
    }
    ...
    private void showBreathingEdge() {
        ...
        int shape = prefs.getInt("breath_shape", 0);
        int w = prefs.getInt("breath_w", 0); int h = prefs.getInt("breath_h", 0);
        int dotSize = prefs.getInt("breath_dot_size", 40);
        int bX = prefs.getInt("breath_dot_x", 0); int bY = prefs.getInt("breath_dot_y", 0);
        ...
        timerHandler.post(new Runnable() {
            @Override public void run() {
                if(!isRecording) return;
                long e = System.currentTimeMillis() - startTime; breathView.setTime(String.format("%02d:%02d", (e/1000)/60, (e/1000)%60));
