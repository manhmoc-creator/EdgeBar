...
import android.Manifest; import android.app.Activity; import android.app.AlertDialog; import android.app.Dialog; import android.content.ComponentName; import android.content.Context; import android.content.Intent; import android.content.SharedPreferences; import android.content.pm.PackageManager; import android.graphics.Color; import android.graphics.drawable.GradientDrawable; import android.os.Bundle; import android.os.Environment; import android.provider.Settings; import android.net.Uri; import android.text.TextUtils; import android.view.Gravity; import android.view.View; import android.view.ViewGroup; import android.widget.*; import java.util.ArrayList;
...
public class MainActivity extends Activity {
    ...
    private LinearLayout pageDesign, pageConditions, pageEcosystem, pageNewbie, listRules, designSliderContainer, navMain, bottomBar; 
    private LinearLayout pageIntents, pageMacros, pageTiles;
    ...
    private final String CURRENT_VERSION = "V19.12.2.2"; 
    ...
    private void refreshPreview() { boolean p = (pageDesign != null && pageDesign.getVisibility()==View.VISIBLE && designTabState==0) || (currentMainTab==1 && currentGesTab==0 && pageNewbie.getVisibility()==View.GONE); prefs.edit().putBoolean("preview_lock", p).apply(); }
    ...
    @Override public void onBackPressed() { 
        if (pageDesign.getVisibility() == View.VISIBLE) { closeDesignSpace(); Button btnD = rootLayout.findViewWithTag("btnDesign"); if(btnD!=null){btnD.setText("⚙️"); btnD.setTextColor(Color.WHITE);} } 
        else if (pageNewbie.getVisibility() == View.VISIBLE) { closeNewbieSpace(); }
        else super.onBackPressed(); 
    }
    private void closeDesignSpace() { pageDesign.setVisibility(View.GONE); navMain.setVisibility(View.VISIBLE); bottomBar.setVisibility(View.VISIBLE); pageConditions.setVisibility(currentMainTab == 1 ? View.VISIBLE : View.GONE); pageEcosystem.setVisibility(currentMainTab == 2 ? View.VISIBLE : View.GONE); refreshPreview(); }
    private void openDesignSpace() { refreshPreview(); navMain.setVisibility(View.GONE); bottomBar.setVisibility(View.VISIBLE); pageConditions.setVisibility(View.GONE); pageEcosystem.setVisibility(View.GONE); pageDesign.setVisibility(View.VISIBLE); }
    
    private void closeNewbieSpace() { pageNewbie.setVisibility(View.GONE); navMain.setVisibility(View.VISIBLE); bottomBar.setVisibility(View.VISIBLE); pageEcosystem.setVisibility(View.VISIBLE); refreshPreview(); }
    private void openNewbieSpace() { refreshPreview(); navMain.setVisibility(View.GONE); bottomBar.setVisibility(View.GONE); pageEcosystem.setVisibility(View.GONE); pageNewbie.setVisibility(View.VISIBLE); }
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        BAR_NAMES = new String[]{T("Bottom Right","Đáy phải"), T("Bottom Left","Đáy trái"), T("Right Edge","Cạnh Phải"), T("Left Edge","Cạnh Trái"), T("Top Center","Đỉnh giữa")}; CORNER_NAMES = new String[]{T("Bottom Right Corner","Góc đáy phải"), T("Bottom Left Corner","Góc đáy trái"), T("Top Right Corner","Góc đỉnh phải"), T("Top Left Corner","Góc đỉnh trái")}; COLOR_NAMES = new String[]{"Trắng Tinh Khiết", "Neon (Pink-Cyan)", "Cyberpunk (Purple-Gold)", "Lava (Red-Orange)", "Ocean (Blue-Cyan)", "Matrix (Green)", "Sunset (Purple-Orange)", "Google (4 Colors)", "Aurora (Cyan-Purple)", "Abyss", "Forest", "Flame", "Midnight", "Tropical", "Candy"};
        ...
        Button btnLang = new Button(this); btnLang.setText(isVi ? "🇻🇳 TIẾNG VIỆT" : "🇺🇸 ENGLISH"); btnLang.setTextColor(Color.WHITE); btnLang.setBackground(getRounded("#2E7D32", 20f)); btnLang.setPadding(30, 20, 30, 20); btnLang.setOnClickListener(v -> { prefs.edit().putBoolean("lang_vi", !isVi).apply(); recreate(); });
        ...
        // V19.12.2.2 NEWBIE SPACE (Giấu trong nút nổi)
        pageNewbie = new LinearLayout(this); pageNewbie.setOrientation(LinearLayout.VERTICAL); pageNewbie.setVisibility(View.GONE); buildNewbieSpace();

        main.addView(pageDesign); main.addView(pageConditions); main.addView(pageEcosystem); main.addView(pageNewbie);
        scroll.addView(main); rootLayout.addView(scroll);

        bottomBar = new LinearLayout(this); bottomBar.setOrientation(LinearLayout.HORIZONTAL); bottomBar.setGravity(Gravity.CENTER_VERTICAL); bottomBar.setBackground(getRounded("#1E1E1E", 100f)); bottomBar.setPadding(20, 20, 20, 20);
        ...
        btnNavCond.setOnClickListener(v->switchMainTab(1, fab)); btnNavAdv.setOnClickListener(v->switchMainTab(2, fab)); 
        fab.setOnClickListener(v -> { if (currentMainTab == 1) openRuleBuilderDialog(null, -1, -1); else openNewbieSpace(); });
        switchMainTab(1, fab); setContentView(rootLayout);
    }

    private void switchMainTab(int idx, Button fab) { 
        currentMainTab = idx; refreshPreview(); navMain.setVisibility(View.VISIBLE);
        pageDesign.setVisibility(View.GONE); pageNewbie.setVisibility(View.GONE); bottomBar.setVisibility(View.VISIBLE);
        ...
        fab.setText(idx == 1 ? "+ NEW EB" : "+ NEWBIE");
    ...
    private void buildEcosystemSpace() { 
        LinearLayout infoCard = new LinearLayout(this); infoCard.setOrientation(LinearLayout.VERTICAL); infoCard.setBackground(getRounded("#1A3B4C", 20f)); infoCard.setPadding(35,35,35,35); LinearLayout.LayoutParams lpI = new LinearLayout.LayoutParams(-1,-2); lpI.setMargins(0,0,0,40); infoCard.setLayoutParams(lpI);
        TextView tInfo = new TextView(this); tInfo.setText(T("💡 CLICK [+ NEWBIE] BELOW TO MANAGE INTENTS, MACROS & QS TILES.\n\nEcosystem components run seamlessly in the background and can be cross-triggered in 1ms.", "💡 HÃY BẤM [+ NEWBIE] BÊN DƯỚI ĐỂ VÀO KHÔNG GIAN INTENTS, MACROS & QS TILES.\n\nCác thành phần hệ sinh thái hoạt động độc lập ngầm và có thể được gán vào Cử chỉ EdgeBar bất cứ lúc nào.")); tInfo.setTextColor(Color.parseColor("#E0F7FA")); tInfo.setTextSize(14); tInfo.setLineSpacing(0, 1.3f); infoCard.addView(tInfo); pageEcosystem.addView(infoCard);
    }

    private void buildNewbieSpace() {
        LinearLayout header = new LinearLayout(this); header.setOrientation(LinearLayout.HORIZONTAL); header.setGravity(Gravity.CENTER_VERTICAL); header.setPadding(0,0,0,40);
        Button btnBack = createCircleBtn("<", "#D32F2F", "#FFFFFF"); btnBack.setOnClickListener(v -> closeNewbieSpace());
        TextView title = new TextView(this); title.setText(T("ECOSYSTEM WORKSPACE", "KHÔNG GIAN HỆ SINH THÁI")); title.setTextColor(Color.parseColor("#00E5FF")); title.setTextSize(20); title.setLayoutParams(new LinearLayout.LayoutParams(0,-2,1f)); title.setPadding(20,0,0,0);
        header.addView(btnBack); header.addView(title); pageNewbie.addView(header);

        LinearLayout tabContainer = new LinearLayout(this); tabContainer.setOrientation(LinearLayout.HORIZONTAL); tabContainer.setPadding(0, 0, 0, 20); 
        Button btnInt = createTabBtn("INTENTS"); Button btnMac = createTabBtn("MACROS"); Button btnQS = createTabBtn("QS TILES");
        LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(0, -2, 1f); p.setMargins(0,0,10,0); btnInt.setLayoutParams(p); btnMac.setLayoutParams(p); btnQS.setLayoutParams(new LinearLayout.LayoutParams(0, -2, 1f));
        tabContainer.addView(btnInt); tabContainer.addView(btnMac); tabContainer.addView(btnQS); pageNewbie.addView(tabContainer);
        ...
        pageNewbie.addView(pageIntents); pageNewbie.addView(pageMacros); pageNewbie.addView(pageTiles);

        View.OnClickListener tabClick = v -> {
            btnInt.setBackground(getRounded(v==btnInt?"#00E5FF":"#222222", 15f)); btnInt.setTextColor(v==btnInt?Color.BLACK:Color.WHITE);
            btnMac.setBackground(getRounded(v==btnMac?"#00E5FF":"#222222", 15f)); btnMac.setTextColor(v==btnMac?Color.BLACK:Color.WHITE);
            btnQS.setBackground(getRounded(v==btnQS?"#00E5FF":"#222222", 15f)); btnQS.setTextColor(v==btnQS?Color.BLACK:Color.WHITE);
            pageIntents.setVisibility(v==btnInt?View.VISIBLE:View.GONE); pageMacros.setVisibility(v==btnMac?View.VISIBLE:View.GONE); pageTiles.setVisibility(v==btnQS?View.VISIBLE:View.GONE);
        };
        btnInt.setOnClickListener(tabClick); btnMac.setOnClickListener(tabClick); btnQS.setOnClickListener(tabClick); btnInt.performClick();
    }
    ...
    private void buildMacrosSpace() {
        pageMacros.addView(createSectionTitle(T("ACCESSIBILITY MULTI-TOGGLE", "ĐÓNG CẮT TRỢ NĂNG HÀNG LOẠT"))); TextView macDesc = new TextView(this); macDesc.setText(T("Enter Package/Class separated by comma.\nEx: com.llamalab.macrodroid/.AccessibilitySvc", "Nhập tên Package/Class ngăn cách bởi dấu phẩy.\nVD: com.llamalab.macrodroid/.AccessibilitySvc")); macDesc.setTextColor(Color.GRAY); macDesc.setPadding(0,0,0,30); pageMacros.addView(macDesc);
        for(int i=1; i<=5; i++) { LinearLayout box = new LinearLayout(this); box.setOrientation(LinearLayout.VERTICAL); box.setPadding(30,10,30,30); box.addView(createInput(T("Macro Name", "Tên gợi nhớ Macro"), "macro_"+i+"_name")); box.addView(createInput(T("Service List", "Danh sách Services"), "macro_"+i+"_svcs")); pageMacros.addView(createDrawer("⚡ MACRO " + i, box)); }
    }

    private void buildTilesSpace() {
        pageTiles.addView(createSectionTitle(T("15 QUICK SETTINGS TILES", "TÙY CHỈNH 15 NÚT QUICK SETTINGS")));
        for(int i=1; i<=15; i++) { 
            ...
            CheckBox cbShow = new CheckBox(this); cbShow.setText(T("Enable Tile in Quick Settings", "Bật Tile này lên Hệ thống (Quick Settings)")); cbShow.setTextColor(Color.parseColor("#4CAF50")); cbShow.setPadding(0,20,0,0);
            ...
            cbShow.setOnCheckedChangeListener((vBtn, isChecked) -> { getPackageManager().setComponentEnabledSetting(comp, isChecked ? PackageManager.COMPONENT_ENABLED_STATE_ENABLED : PackageManager.COMPONENT_ENABLED_STATE_DISABLED, PackageManager.DONT_KILL_APP); Toast.makeText(this, isChecked ? T("Tile Enabled!", "Đã MỞ KHÓA Tile") : T("Tile Hidden", "Đã ẨN Tile"), Toast.LENGTH_SHORT).show(); });
    ...
    private void buildDesignSpace() {
        ...
        LinearLayout backupRow = new LinearLayout(this); backupRow.setOrientation(LinearLayout.HORIZONTAL); Button btnBackup = new Button(this); btnBackup.setText(T("💾 BACKUP", "💾 SAO LƯU")); btnBackup.setBackground(getRounded("#2E7D32", 20f)); btnBackup.setTextColor(Color.WHITE); LinearLayout.LayoutParams bp = new LinearLayout.LayoutParams(0, -2, 1f); bp.setMargins(0,0,15,0); btnBackup.setLayoutParams(bp); Button btnRestore = new Button(this); btnRestore.setText(T("📂 RESTORE", "📂 PHỤC HỒI")); btnRestore.setBackground(getRounded("#EF6C00", 20f)); btnRestore.setTextColor(Color.WHITE); LinearLayout.LayoutParams rp = new LinearLayout.LayoutParams(0, -2, 1f); btnRestore.setLayoutParams(rp); btnBackup.setOnClickListener(v -> { Intent i = new Intent(Intent.ACTION_CREATE_DOCUMENT); i.addCategory(Intent.CATEGORY_OPENABLE); i.setType("text/plain"); i.putExtra(Intent.EXTRA_TITLE, "EdgeBar_Backup.txt"); startActivityForResult(i, 101); }); btnRestore.setOnClickListener(v -> { Intent i = new Intent(Intent.ACTION_OPEN_DOCUMENT); i.addCategory(Intent.CATEGORY_OPENABLE); i.setType("*/*"); startActivityForResult(i, 102); }); backupRow.addView(btnBackup); backupRow.addView(btnRestore); pageDesign.addView(wrapCard(backupRow));
        ...
        // V19.12.2.2: CẤU HÌNH GHI ÂM (VOICE RECORDER) WITH CUSTOM RED EDGE BREATH
        ...
        recSys.addView(createComboDropdown(T("Breath Color","Màu sắc Hơi thở"), "rec_color", new String[]{T("Red","Đỏ"), T("Green","Lục"), T("Blue","Lam")}, 0));
        recSys.addView(createSlider(T("Breath Flash (ms)", "Thời gian nhịp sáng (ms)"), "rec_anim_dur", 3000, 1200));
        recSys.addView(createSlider(T("Breath Gap (ms)", "Thời gian ngắt quãng (ms)"), "rec_anim_gap", 3000, 800));
        Button btnTestRec = new Button(this); btnTestRec.setText(T("▶ TEST BREATH", "▶ THỬ HƠI THỞ GHI ÂM")); btnTestRec.setBackground(getRounded("#FFC107", 20f)); btnTestRec.setTextColor(Color.BLACK); btnTestRec.setPadding(0,30,0,30); LinearLayout.LayoutParams testLp2 = new LinearLayout.LayoutParams(-1,-2); testLp2.setMargins(0,20,0,20); btnTestRec.setLayoutParams(testLp2); 
        btnTestRec.setOnClickListener(v -> { Intent i = new Intent(this, RecorderService.class); i.setAction("TEST_BREATH"); startService(i); }); recSys.addView(btnTestRec);
        TextView txtPath = new TextView(this); txtPath.setText(T("Save to: Documents/EdgeBar_Records", "Lưu tại: Documents/EdgeBar_Records")); txtPath.setTextColor(Color.GRAY); recSys.addView(txtPath);
    ...
    private void showPremiumDialog() { 
        Dialog d = new Dialog(this, android.R.style.Theme_DeviceDefault_NoActionBar_Fullscreen); ScrollView sv = new ScrollView(this); sv.setPadding(50,50,50,50); sv.setBackgroundColor(Color.parseColor("#121212")); LinearLayout root = new LinearLayout(this); root.setOrientation(LinearLayout.VERTICAL); sv.addView(root);
        TextView title = new TextView(this); title.setText("👑 PREMIUM ARCHITECT INFO"); title.setTextColor(Color.parseColor("#00E5FF")); title.setTextSize(22); title.setPadding(0,0,0,30); root.addView(title);
        TextView diff = new TextView(this); diff.setText(T("💡 KHÁC BIỆT CỐT LÕI:\n• Màn hình chính (Homescreen): Hoạt động bằng Overlay, KHÔNG dùng Trợ năng -> Tàng hình, mượt mà.\n• Màn khóa (Lockscreen): Dùng Trợ năng để xuyên thấu bảo mật.\n• Hệ sinh thái: Tốc độ bắn lệnh chéo 1ms.", "💡 KHÁC BIỆT CỐT LÕI:\n• Màn hình chính (Homescreen): Hoạt động bằng Overlay, KHÔNG dùng Trợ năng -> Tàng hình, mượt mà.\n• Màn khóa (Lockscreen): Dùng Trợ năng để xuyên thấu bảo mật.\n• Hệ sinh thái: Tốc độ bắn lệnh chéo 1ms.")); diff.setTextColor(Color.parseColor("#E0F7FA")); diff.setLineSpacing(0, 1.3f); diff.setPadding(0,0,0,40); root.addView(diff);
        TextView adbTitle = new TextView(this); adbTitle.setText(T("🔧 LỆNH ADB CỐT LÕI (Cấp 1 lần):", "🔧 LỆNH ADB CỐT LÕI (Cấp 1 lần):")); adbTitle.setTextColor(Color.parseColor("#E91E63")); adbTitle.setPadding(0,0,0,20); root.addView(adbTitle);
        EditText cmd1 = new EditText(this); cmd1.setText("adb shell pm grant com.manhmoc.edgebar android.permission.WRITE_SECURE_SETTINGS"); cmd1.setTextColor(Color.WHITE); cmd1.setBackground(getRounded("#1E1E1E", 15f)); cmd1.setPadding(30,30,30,30); LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(-1,-2); lp.setMargins(0,0,0,20); cmd1.setLayoutParams(lp); root.addView(cmd1);
        EditText cmd2 = new EditText(this); cmd2.setText("adb shell appops set com.manhmoc.edgebar SYSTEM_ALERT_WINDOW allow"); cmd2.setTextColor(Color.WHITE); cmd2.setBackground(getRounded("#1E1E1E", 15f)); cmd2.setPadding(30,30,30,30); cmd2.setLayoutParams(lp); root.addView(cmd2);
        TextView adbHelp = new TextView(this); adbHelp.setText(T("👉 Hướng dẫn chạy ADB:\n1. Tải Platform-tools về máy tính.\n2. Vào Cài đặt điện thoại -> Giới thiệu -> Bấm 7 lần vào Số bản dựng để Bật Tùy chọn Nhà phát triển.\n3. Bật 'Gỡ lỗi USB' (USB Debugging).\n4. Cắm cáp, mở CMD tại thư mục platform-tools, copy paste 2 lệnh trên.\n\n💡 NẾU CÓ ADB, APP SẼ:\n- Tự cấp quyền Lớp phủ.\n- Đóng cắt Trợ năng siêu tốc.", "👉 Hướng dẫn chạy ADB:\n1. Tải Platform-tools về máy tính.\n2. Vào Cài đặt điện thoại -> Giới thiệu -> Bấm 7 lần vào Số bản dựng để Bật Tùy chọn Nhà phát triển.\n3. Bật 'Gỡ lỗi USB' (USB Debugging).\n4. Cắm cáp, mở CMD tại thư mục platform-tools, copy paste 2 lệnh trên.\n\n💡 NẾU CÓ ADB, APP SẼ:\n- Tự cấp quyền Lớp phủ.\n- Đóng cắt Trợ năng siêu tốc.")); adbHelp.setTextColor(Color.GRAY); adbHelp.setLineSpacing(0, 1.2f); root.addView(adbHelp);
        Button btnClose = new Button(this); btnClose.setText(T("CLOSE", "ĐÓNG")); btnClose.setBackground(getRounded("#333333", 20f)); btnClose.setTextColor(Color.WHITE); LinearLayout.LayoutParams bLp = new LinearLayout.LayoutParams(-1,-2); bLp.setMargins(0,50,0,50); btnClose.setLayoutParams(bLp); btnClose.setOnClickListener(v->d.dismiss()); root.addView(btnClose);
    ...
    private void renderSliders() { designSliderContainer.removeAllViews(); if(designTabState == 2) { 
        ...
        LinearLayout lS = new LinearLayout(this); lS.setOrientation(LinearLayout.HORIZONTAL); lS.setPadding(0,10,0,10); TextView tS = new TextView(this); tS.setText(T("Style:","Kiểu chạy:")); tS.setTextColor(Color.WHITE); tS.setLayoutParams(new LinearLayout.LayoutParams(0,-2,1f)); Spinner sS = createSpinner(); sS.setAdapter(new ArrayAdapter<>(this, android.R.layout.simple_spinner_dropdown_item, new String[]{T("Blink","Nhấp Nháy"), T("1 Tail","1 Tia sáng nối đuôi"), T("2 Tails","2 Tia sáng đối xứng"), T("3 Tails","3 Tia sáng đều nhau")})); sS.setSelection(prefs.getInt("anim_style", 0)); sS.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener(){public void onItemSelected(AdapterView<?> p, View v, int pos, long id){prefs.edit().putInt("anim_style", pos).apply();}public void onNothingSelected(AdapterView<?> p){}}); sS.setLayoutParams(new LinearLayout.LayoutParams(0,-2,1.5f)); lS.addView(tS); lS.addView(sS); designSliderContainer.addView(lS); 
        designSliderContainer.addView(createSlider(T("Animation Width", "Chiều ngang Hiệu ứng (0=Full)"), "anim_w", 2000, 0)); designSliderContainer.addView(createSlider(T("Animation Height", "Chiều dọc Hiệu ứng (0=Full)"), "anim_h", 3500, 0)); designSliderContainer.addView(createSlider(T("Alpha", "Độ đậm mờ hiệu ứng (Alpha)"), "anim_alpha", 255, 255)); designSliderContainer.addView(createSlider(T("Thickness", "Độ dày viền"), "anim_thick", 50, 12)); designSliderContainer.addView(createSlider(T("Animation Dur (ms)", "Thời gian Animation (ms)"), "anim_dur", 5000, 1500)); designSliderContainer.addView(createSlider(T("Hold Dur (ms)", "Thời gian Vuốt+Giữ (All)"), "hold_dur", 2000, 600)); designSliderContainer.addView(createSlider(T("Vibrate (ms)", "Độ rung (ms) (All)"), "vib_dur", 100, 30)); } else { String prefix = designTabState == 1 ? "home_" : "lock_"; 
        designSliderContainer.addView(createSectionTitle("5 EDGE BARS"));
        for(int i=0; i< 5; i++) { LinearLayout drawerContent = new LinearLayout(this); drawerContent.setOrientation(LinearLayout.VERTICAL); drawerContent.setPadding(30,10,30,30); CheckBox cb = new CheckBox(this); cb.setText(T("ENABLE: ", "BẬT: ") + BAR_NAMES[i]); cb.setTextColor(Color.parseColor("#4CAF50")); cb.setChecked(prefs.getBoolean(prefix+BARS[i]+"_en", i < 2)); final int idx = i; cb.setOnCheckedChangeListener((v,c) -> prefs.edit().putBoolean(prefix+BARS[idx]+"_en", c).apply()); drawerContent.addView(cb); drawerContent.addView(createComboDropdown(T("Touch Mode", "Chế độ Cảm ứng"), prefix+BARS[i]+"_pri_mode", new String[]{T("Priority (Lock)", "Ưu tiên Edge Bar (Khoá cứng)"), T("Pass-through", "Nhường OS (Xuyên thấu)")}, 0)); drawerContent.addView(createSlider(T("Alpha", "Độ trong suốt"), prefix+BARS[i]+"_alpha", 255, 50)); drawerContent.addView(createSlider(T("Width", "Chiều ngang"), prefix+BARS[i]+"_w", 3000, 300)); drawerContent.addView(createSlider(T("Height", "Chiều dọc"), prefix+BARS[i]+"_h", 3000, 60)); drawerContent.addView(createSlider(T("X Pos", "Toạ độ X"), prefix+BARS[i]+"_x", 1000, 0)); drawerContent.addView(createSlider(T("Y Pos", "Toạ độ Y"), prefix+BARS[i]+"_y", 1000, 0)); designSliderContainer.addView(createDrawer(BAR_NAMES[i], drawerContent)); } 
        designSliderContainer.addView(createSectionTitle("4 FRAME CORNERS"));
        for(int i=0; i<4; i++) { LinearLayout drawerContent = new LinearLayout(this); drawerContent.setOrientation(LinearLayout.VERTICAL); drawerContent.setPadding(30,10,30,30); drawerContent.addView(createComboDropdown(T("Visibility", "Hiển thị"), prefix+"corner_"+CORNERS[i]+"_vis_mode", new String[]{T("Visible", "Hiện hoàn toàn"), T("Stealth (Blink)", "Tàng hình (Nháy sáng)"), T("Invisible", "Ẩn hoàn toàn (Vô hình)")}, 0)); drawerContent.addView(createComboDropdown(T("Touch Mode", "Chế độ Cảm ứng"), prefix+"corner_"+CORNERS[i]+"_pri_mode", new String[]{T("Priority (Lock)", "Ưu tiên Edge Bar (Khoá cứng)"), T("Pass-through", "Nhường OS (Xuyên thấu)")}, 0)); drawerContent.addView(createComboDropdown(T("Shape", "Hình dáng Góc"), prefix+"corner_"+CORNERS[i]+"_shape", new String[]{T("Curve", "Bo Cong"), T("Horiz Straight", "Thẳng Ngang"), T("Vert Straight", "Thẳng Dọc")}, 0)); drawerContent.addView(createSlider(T("Outer Width (X)", "Kéo giãn Ngang Vỏ (X)"), prefix+"corner_"+CORNERS[i]+"_w", 2500, 100)); drawerContent.addView(createSlider(T("Outer Height (Y)", "Kéo giãn Dọc Vỏ (Y)"), prefix+"corner_"+CORNERS[i]+"_h", 2500, 100)); drawerContent.addView(createSlider(T("Move X", "Di chuyển Ngang (X)"), prefix+"corner_"+CORNERS[i]+"_x", 2500, 0)); drawerContent.addView(createSlider(T("Move Y", "Di chuyển Dọc (Y)"), prefix+"corner_"+CORNERS[i]+"_y", 2500, 0)); drawerContent.addView(createSlider(T("Inner Width (X)", "Kéo giãn Ngang Lõi Trăng Non (X)"), prefix+"corner_"+CORNERS[i]+"_moon_w", 2500, 100)); drawerContent.addView(createSlider(T("Inner Height (Y)", "Kéo giãn Dọc Lõi Trăng Non (Y)"), prefix+"corner_"+CORNERS[i]+"_moon_h", 2500, 100)); drawerContent.addView(createSlider(T("Inner X (1250=Center)", "Di chuyển Trăng Non Ngang (X) (1250=Giữa)"), prefix+"corner_"+CORNERS[i]+"_moon_x", 2500, 1250)); drawerContent.addView(createSlider(T("Inner Y (1250=Center)", "Di chuyển Trăng Non Dọc (Y) (1250=Giữa)"), prefix+"corner_"+CORNERS[i]+"_moon_y", 2500, 1250)); drawerContent.addView(createSlider(T("Outer Radius", "Độ cong BO VIỀN (Vỏ) (1000=Thẳng)"), prefix+"corner_"+CORNERS[i]+"_rad", 1000, 80)); drawerContent.addView(createSlider(T("Inner Radius", "Độ cong TRĂNG NON (Lõi) (1000=Thẳng)"), prefix+"corner_"+CORNERS[i]+"_moon_rad", 1000, 80)); designSliderContainer.addView(createDrawer(CORNER_NAMES[i], drawerContent)); }
        LinearLayout globalDrawer = new LinearLayout(this); globalDrawer.setOrientation(LinearLayout.VERTICAL); globalDrawer.setPadding(30,10,30,30); globalDrawer.addView(createSlider(T("Stealth Hide Delay (ms)", "Thời gian chờ tắt tàng hình (ms)"), prefix+"corner_hide_dur", 5000, 2500)); globalDrawer.addView(createSlider(T("Inner Alpha", "Độ mờ vùng TRĂNG NON (Đậm/Nhạt)"), prefix+"corner_moon_alpha", 255, 100)); globalDrawer.addView(createSlider(T("Outer Alpha", "Độ mờ VIỀN GÓC (Đậm/Nhạt)"), prefix+"corner_stroke_alpha", 255, 200)); globalDrawer.addView(createSlider(T("Thickness", "Độ đậm viền (Dày/Mỏng)"), prefix+"corner_thick", 50, 8)); designSliderContainer.addView(createDrawer(T("GLOBAL CORNER CONFIG", "TÙY CHỈNH CHUNG GÓC VIỀN"), globalDrawer));
        } }
