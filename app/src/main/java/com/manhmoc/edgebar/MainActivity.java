...
public class MainActivity extends Activity {
    ...
    private void openMorseMapperDialog() {
        ...
        String[] allMorseComponents = {"r", "l", "t_r", "t_l", "t_c", "m_b_c", "m_mid_t", "m_mid_b", "tl", "tr", "bl", "br"};
        String[] displayNames = {
                ...
                T("Corner TL", "Góc đỉnh trái"), T("Corner TR", "Góc đỉnh phải"), T("Corner BL", "Góc đáy trái"), T("Corner BR", "Góc đáy phải")
        };
    ...
    private void buildIntentsSpace() {
        for (int i = 1; i <= 15; i++) {
            ...
            sInt.addView(createInput(T("Alias (e.g., Open FB)", "Tên gợi nhớ (VD: Mở Facebook)"), "intent_"+i+"_name"));
            sInt.addView(createInput("Action", "i"+i+"_act"));
            sInt.addView(createInput("Package", "i"+i+"_pkg"));
            sInt.addView(createInput("Class Name", "i"+i+"_cls"));
            sInt.addView(createInput("Data URI (acc://pkg/.cls)", "i"+i+"_data"));
            sInt.addView(createInput("Categories", "i"+i+"_cat"));
            sInt.addView(createInput("Flags", "i"+i+"_flags"));
            CheckBox cb = new CheckBox(this); cb.setText(T("Send as Broadcast", "Gửi dạng Broadcast")); cb.setTextColor(Color.WHITE);
            cb.setChecked(prefs.getBoolean("i"+i+"_br", true)); final int idx = i;
            cb.setOnCheckedChangeListener((v,c) -> prefs.edit().putBoolean("i"+idx+"_br", c).apply()); sInt.addView(cb);
            pageIntents.addView(createDrawer("⚙ Intent Slot " + i, sInt));
        }
    }

    private void buildMacrosSpace() {
        pageMacros.addView(createSectionTitle(T("ACCESSIBILITY MACROS", "ĐÓNG CẮT TRỢ NĂNG HÀNG LOẠT")));
        TextView macDesc = new TextView(this); macDesc.setText(T("Format: com.pkg/.Service, com.pkg2/.Service", "Nhập tên Package/Class ngăn cách bởi dấu phẩy.\nVD: com.llamalab.macrodroid/.AccessibilitySvc"));
        macDesc.setTextColor(Color.GRAY); macDesc.setPadding(0,0,0,30); pageMacros.addView(macDesc);
        for(int i=1; i<=5; i++) {
            LinearLayout box = new LinearLayout(this); box.setOrientation(LinearLayout.VERTICAL); box.setPadding(30,10,30,30);
            box.addView(createInput(T("Macro Name", "Tên gợi nhớ Macro"), "macro_"+i+"_name"));
            box.addView(createInput(T("Target Services", "Danh sách Services"), "macro_"+i+"_svcs"));
    ...
    private void buildTilesSpace() {
        ...
        for(int i=1; i<=15; i++) {
            ...
            String v = prefs.getString("tile_"+i+"_act","NONE"); for(int j=0;j<ACT_KEYS.length;j++) if(ACT_KEYS[j].equals(v)) s.setSelection(j);
            final int idx=i; s.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener(){
                public void onItemSelected(AdapterView<?> p, View v, int pos, long id){ prefs.edit().putString("tile_"+idx+"_act",ACT_KEYS[pos]).apply(); }
                public void onNothingSelected(AdapterView<?> p){}
            }); s.setLayoutParams(new LinearLayout.LayoutParams(0,-2,1.5f));
            row.addView(tv); row.addView(s); content.addView(row);

            CheckBox cbShow = new CheckBox(this); cbShow.setText(T("Enable this Tile", "Bật Tile này lên Hệ thống (Quick Settings)"));
            cbShow.setTextColor(Color.parseColor("#4CAF50")); cbShow.setPadding(0,20,0,0);
            ...
            cbShow.setOnCheckedChangeListener((vBtn, isChecked) -> {
                ...
                Toast.makeText(this, isChecked ? T("Tile unlocked!", "Đã MỞ KHÓA Tile "+idx) : T("Tile hidden!", "Đã ẨN Tile "+idx), Toast.LENGTH_LONG).show();
            });
            content.addView(cbShow);
            pageTiles.addView(createDrawer("🔲 QS Tile " + i, content));
        }
    }
    ...
    private void buildDesignSpace() {
        ...
        LinearLayout.LayoutParams rp = new LinearLayout.LayoutParams(0, -2, 1f); btnRestore.setLayoutParams(rp);
        ...
        CheckBox cbKbd = new CheckBox(this); cbKbd.setText(T("Auto-hide on Keyboard", "Tự ẩn khi hiện Bàn Phím")); cbKbd.setTextColor(Color.WHITE);
        cbKbd.setChecked(prefs.getBoolean("avoid_kbd", true)); cbKbd.setOnCheckedChangeListener((v,c) -> prefs.edit().putBoolean("avoid_kbd", c).apply());
    ...
    private void showPremiumDialog() {
        String t = T("ADB COMMANDS:\nadb shell pm grant com.manhmoc.edgebar android.permission.WRITE_SECURE_SETTINGS\nadb shell appops set com.manhmoc.edgebar SYSTEM_ALERT_WINDOW allow\n\n🚀 V19.12.3.4.5.3:\n- Fixed all crashes (Flash, Clipboard, Camera)\n- Real App Picker for Locklist\n- Morse corner mapping fixed\n- X and > keys work correctly\n- Morse overlay toggle button\n- Touch through Morse when designing", "🔧 LỆNH ADB CỐT LÕI:\n\n1. Quyền ghi Cài đặt bảo mật:\nadb shell pm grant com.manhmoc.edgebar android.permission.WRITE_SECURE_SETTINGS\n\n2. Quyền vẽ Lớp phủ (Tàng hình AppOps):\nadb shell appops set com.manhmoc.edgebar SYSTEM_ALERT_WINDOW allow\n\n🚀 V19.12.3.4.5.3:\n- Sửa crash (Đèn pin, Camera, Clipboard)\n- App Picker thật cho Locklist\n- Sửa ánh xạ góc Morse\n- Phím X và > hoạt động đúng\n- Nút bật/tắt lớp phủ Morse\n- Chạm xuyên qua khi thiết kế");
    ...
    private LinearLayout createDrawer(String title, View content) {
        ...
        LinearLayout.LayoutParams clp = new LinearLayout.LayoutParams(-1,-2); clp.setMargins(0,0,0,20);
        container.setLayoutParams(clp);
        TextView header = new TextView(this); header.setText(title); header.setTextColor(Color.parseColor("#00E5FF"));
        header.setPadding(30,30,30,30); header.setTextSize(16);
    ...
    private LinearLayout createComboDropdown(String title, String key, String[] items, int def) {
        ...
        Spinner sp = createSpinner(); sp.setAdapter(new ArrayAdapter<>(this, android.R.layout.simple_spinner_dropdown_item, items));
    ...
    private void renderSliders() {
        ...
        if (designTabState == 3) {
            ...
            designSliderContainer.addView(createSlider("Chiều ngang Hiệu ứng (0=Full)", "anim_w", 2000, 0));
            designSliderContainer.addView(createSlider("Chiều dọc Hiệu ứng (0=Full)", "anim_h", 3500, 0));
            designSliderContainer.addView(createSlider("Độ đậm mờ hiệu ứng (Alpha)", "anim_alpha", 255, 255));
            designSliderContainer.addView(createSlider("Độ dày viền", "anim_thick", 50, 12));
            designSliderContainer.addView(createSlider("Thời gian Animation (ms)", "anim_dur", 5000, 1500));
            designSliderContainer.addView(createSlider("Thời gian Vuốt+Giữ (All)", "hold_dur", 2000, 600));
            designSliderContainer.addView(createSlider("Độ rung (ms) (All)", "vib_dur", 100, 30));
        } else {
            ...
            if (designTabState == 2) {
                // NÚT BẬT/TẮT LỚP PHỦ MORSE TRONG DESIGN SPACE
                ...
                etPass.setText(prefs.getString("morse_master_pass", "").replace(">", ""));
                ...
                btnSavePass.setOnClickListener(v -> {
                    String pass = etPass.getText().toString().trim().replace(">", "");
            ...
            for (int i = 0; i < bKeys.length; i++) {
                ...
                final int idx = i; cb.setOnCheckedChangeListener((v,c) -> prefs.edit().putBoolean(prefix + bKeys[idx] + "_en", c).apply());
            ...
            for (int i = 0; i < 4; i++) {
                LinearLayout drawerContent = new LinearLayout(this); drawerContent.setOrientation(LinearLayout.VERTICAL); drawerContent.setPadding(30,10,30,30);
                CheckBox cbEn = new CheckBox(this); cbEn.setText(T("ENABLE: ", "BẬT: ") + CORNER_NAMES[i]); cbEn.setTextColor(Color.parseColor("#4CAF50"));
                cbEn.setChecked(prefs.getBoolean(prefix + "corner_" + CORNERS[i] + "_en", false));
                final int idx = i; cbEn.setOnCheckedChangeListener((v,c) -> prefs.edit().putBoolean(prefix + "corner_" + CORNERS[idx] + "_en", c).apply());
