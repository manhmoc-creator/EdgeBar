...
public class MainActivity extends Activity {
    ...
    private final String CURRENT_VERSION = "V19.12.3.4.4"; private RelativeLayout rootLayout;
    ...
    private void refreshPreview() { boolean pLock = (pageDesign != null && pageDesign.getVisibility()==View.VISIBLE && designTabState==0) || (currentMainTab==1 && currentGesTab==0); boolean pMorse = (pageDesign != null && pageDesign.getVisibility()==View.VISIBLE && designTabState==2); prefs.edit().putBoolean("preview_lock", pLock).putBoolean("preview_morse", pMorse).apply(); Intent i = new Intent("com.manhmoc.edgebar.SYNC_STATE"); sendBroadcast(i); }
    ...
    private void buildConditionsSpace() {
        ...
        LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(0,-2,1f); p.setMargins(0,0,15,0); btnLock.setLayoutParams(p); btnHome.setLayoutParams(new LinearLayout.LayoutParams(0,-2,1f));
    ...
    private View buildRuleEditor(Dialog dialog, String editKey, int preComp, int preGes) {
        ...
        for (int i=1; i<ACT_LABS.length; i++) { CheckBox cbAct = new CheckBox(this); cbAct.setText(ACT_LABS[i]); cbAct.setTextColor(Color.WHITE); cbAct.setPadding(0,20,0,20); boolean isChecked = false; for(String sa : savedArray) { if(sa.trim().equals(ACT_KEYS[i])) { isChecked = true; break; } } cbAct.setChecked(isChecked); actionBoxes.add(cbAct); vAct.addView(cbAct); }
