...
public class MainActivity extends Activity {
    ...
    private void renderSliders() { 
        ...
        if(designTabState == 3) { 
            ...
            if(designTabState == 2) {
                Button btnRecord = new Button(this); btnRecord.setText(T("MAP MORSE KEYS", "GÁN MÃ SỐ CHO CÁC PHÍM")); btnRecord.setBackground(getRounded("#E91E63", 20f)); btnRecord.setTextColor(Color.WHITE); btnRecord.setPadding(0,30,0,30); LinearLayout.LayoutParams recLp = new LinearLayout.LayoutParams(-1,-2); recLp.setMargins(0,0,0,20); btnRecord.setLayoutParams(recLp);
                btnRecord.setOnClickListener(v -> openMorseMapper());
    ...
    private void openMorseMapper() {
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        View view = getLayoutInflater().inflate(android.R.layout.linear_layout, null);
        LinearLayout layout = (LinearLayout) view; layout.setOrientation(LinearLayout.VERTICAL);
        String[] components = {"Thanh Đáy Phải (1)", "Thanh Đáy Trái (2)", "Cạnh Phải (3)", "Cạnh Trái (4)", "Đỉnh Giữa (5)", "Đáy Giữa (6)", "Trung Tâm Trên (7)", "Trung Tâm Dưới (8)", "Góc Đáy Phải (Xóa)", "Góc Đáy Trái (Lưu)", "Góc Đỉnh Phải", "Góc Đỉnh Trái"};
        final EditText[] edits = new EditText[components.length];
        for (int i=0; i<components.length; i++) {
            LinearLayout row = new LinearLayout(this); row.setOrientation(LinearLayout.HORIZONTAL); row.setPadding(0,20,0,20);
            TextView label = new TextView(this); label.setText(components[i]); label.setTextColor(Color.WHITE); label.setLayoutParams(new LinearLayout.LayoutParams(0,-2,1f));
            edits[i] = new EditText(this); edits[i].setHint("Số/Chữ"); edits[i].setText(prefs.getString("morse_map_"+components[i].substring(0,5), ""));
            row.addView(label); row.addView(edits[i]); layout.addView(row);
        }
        builder.setTitle("Gán mã cho các phím Morse").setView(view).setPositiveButton("Lưu", (d, which) -> {
            for (int i=0; i<components.length; i++) {
                String key = "morse_map_"+components[i].substring(0,5);
                prefs.edit().putString(key, edits[i].getText().toString()).apply();
            }
