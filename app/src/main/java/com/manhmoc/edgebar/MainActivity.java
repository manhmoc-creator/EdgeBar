...
public class MainActivity extends Activity {
    ...
    // V19.11.4.1: ĐẢO THỨ TỰ ƯU TIÊN PHẢI TRƯỚC, ĐÁY TRƯỚC
    private String[] BARS = {"r", "l", "t_r", "t_l", "t_c"}; 
    private String[] BAR_NAMES; 
    ...
    private int designTabState = 0; private final String CURRENT_VERSION = "V19.11.4.1"; 
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView title = new TextView(this); title.setText("⚙️ Edge Bar " + CURRENT_VERSION + "\nThe Organizer"); title.setTextColor(Color.WHITE); title.setTextSize(21); title.setLayoutParams(new LinearLayout.LayoutParams(0, -2, 1f));
        ...
        for (int i = 1; i <= 15; i++) { LinearLayout sInt = new LinearLayout(this); sInt.setOrientation(LinearLayout.VERTICAL); sInt.addView(createSectionTitle("Intent Slot " + i)); sInt.addView(createInput("Action", "i"+i+"_act")); sInt.addView(createInput("Package", "i"+i+"_pkg")); sInt.addView(createInput("Class Name", "i"+i+"_cls")); sInt.addView(createInput("Data URI", "i"+i+"_data")); sInt.addView(createInput("Categories", "i"+i+"_cat")); sInt.addView(createInput("Flags", "i"+i+"_flags")); CheckBox cb = new CheckBox(this); cb.setText(T("Send as Broadcast", "Gửi dạng Broadcast")); cb.setTextColor(Color.WHITE); cb.setChecked(prefs.getBoolean("i"+i+"_br", true)); final int idx = i; cb.setOnCheckedChangeListener((v,c) -> prefs.edit().putBoolean("i"+idx+"_br", c).apply()); sInt.addView(cb); pageIntents.addView(wrapCard(sInt)); }
    ...
    // V19.11.4.1 HÀM TẠO NGĂN KÉO TỦ (Accordion/Drawer)
    private LinearLayout createDrawer(String title, View content) {
        ...
        content.setVisibility(View.GONE); // Mặc định đóng ngăn kéo
        
        header.setOnClickListener(v -> {
            boolean isClosed = content.getVisibility() == View.GONE;
            content.setVisibility(isClosed ? View.VISIBLE : View.GONE);
            header.setText((isClosed ? "▲ " : "▼ ") + title);
            header.setBackground(getRounded(isClosed ? "#333333" : "#222222", 20f));
        });
    ...
    private void renderSliders() { designSliderContainer.removeAllViews(); if(designTabState == 2) { 
        ...
        for(int i=0; i< BAR_NAMES.length; i++) { 
            ...
            // V19.11.4.1: THÊM CƠ CHẾ XUYÊN THẤU CẢM ỨNG (PASS-THROUGH)
        ...
        // V19.11.4.1 TĂNG MAX LÊN 2500 ĐỂ CHO PHÉP THU NHỎ VỀ 10PX
    ...
    private LinearLayout createConfigPage(String prefix) { LinearLayout page = new LinearLayout(this); page.setOrientation(LinearLayout.VERTICAL); for(int i=0; i<5; i++) { LinearLayout sec = new LinearLayout(this); sec.setOrientation(LinearLayout.VERTICAL); for(int j=0; j<GESTURES.length; j++) sec.addView(createRow(GESTURE_NAMES[j], prefix + "_" + BARS[i] + "_" + GESTURES[j])); page.addView(createDrawer(BAR_NAMES[i], sec)); } LinearLayout secC = new LinearLayout(this); secC.setOrientation(LinearLayout.VERTICAL); secC.addView(createSectionTitle(T("4 FRAME CORNERS","4 GÓC VIỀN"))); for(int i=0; i<4; i++) { LinearLayout secGoc = new LinearLayout(this); secGoc.setOrientation(LinearLayout.VERTICAL); for(int j=0; j<C_GESTURES.length; j++) secGoc.addView(createRow(C_GESTURE_NAMES[j], prefix + "_corner_" + CORNERS[i] + "_" + C_GESTURES[j])); secC.addView(createDrawer(CORNER_NAMES[i], secGoc)); } page.addView(wrapCard(secC)); return page; }
