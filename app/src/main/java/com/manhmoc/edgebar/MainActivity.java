...
import android.app.Activity; import android.app.AlertDialog; import android.app.Dialog; import android.content.Context; import android.content.Intent; import android.content.SharedPreferences; import android.content.pm.PackageManager; import android.content.pm.ResolveInfo; import android.graphics.Color; import android.graphics.drawable.GradientDrawable; import android.os.Bundle; import android.provider.Settings; import android.net.Uri; import android.text.TextUtils; import android.view.Gravity; import android.view.View; import android.view.ViewGroup; import android.widget.*; import org.json.JSONObject; import java.util.ArrayList; import java.util.Arrays; import java.util.Iterator; import java.util.List;
...
public class MainActivity extends Activity {
    ...
    private final String CURRENT_VERSION = "V19.12.3.4.8"; 
    ...
    @Override protected void onPause() { super.onPause(); prefs.edit().putBoolean("preview_lock", false).putBoolean("preview_morse", false).apply(); sendBroadcast(new Intent("com.manhmoc.edgebar.SYNC_STATE")); }
    ...
    private void reloadActionLabels() {
        ...
        M_BAR_NAMES = new String[]{T("Bottom Right", "Đáy phải"), T("Bottom Left", "Đáy trái"), T("Top Right", "Cạnh Phải"), T("Top Left", "Cạnh Trái"), T("Top Center", "Đỉnh giữa"), T("Bottom Center", "Đáy Giữa"), T("Top Half", "Trung Tâm Trên (Chỉnh Y)"), T("Bottom Half", "Trung Tâm Dưới (Chỉnh Y)")};
    ...
    private void showAppPickerDialog() {
        ...
        builder.setTitle(T("Choose App", "Chọn ứng dụng cần khóa"));
        ...
        List<String> selectedApps = new ArrayList<>(Arrays.asList(currentLocklist.split(",")));
        for (ResolveInfo app : apps) {
            CheckBox cb = new CheckBox(this); cb.setText(app.loadLabel(getPackageManager())); cb.setTextColor(Color.WHITE);
            cb.setChecked(selectedApps.contains(app.activityInfo.packageName));
            cb.setOnCheckedChangeListener((v, isChecked) -> {
                if (isChecked) selectedApps.add(app.activityInfo.packageName);
                else selectedApps.remove(app.activityInfo.packageName);
            });
            layout.addView(cb);
        }
        ScrollView sv = new ScrollView(this); sv.addView(layout); builder.setView(sv);
        ...
        builder.setNegativeButton("HỦY", null); builder.show();
    }
    ...
    private void openMorseMapDialog() {
        ...
        TextView title = new TextView(this); title.setText("🔢 BẢN ĐỒ PHÍM SỐ MORSE"); title.setTextColor(Color.parseColor("#00E5FF")); title.setTextSize(20); title.setPadding(0,0,0,40); root.addView(title);
        ...
        String[] mKeys = {"t_l", "m_mid_t", "t_r", "l", "t_c", "r", "tl", "tr", "bl", "br", "m_b_c", "m_mid_b"};
        String[] mNames = {"Cạnh Trái", "Trung Tâm Trên", "Cạnh Phải", "Đáy Trái", "Đỉnh Giữa", "Đáy Phải", "Góc Đỉnh Trái", "Góc Đỉnh Phải", "Góc Đáy Trái (Khuyên dùng X)", "Góc Đáy Phải (Khuyên dùng >)", "Đáy Giữa", "Trung Tâm Dưới"};
        String[] mapOptions = {"* (Bỏ qua)", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "X (Xóa)", "> (Enter Mở khóa)"};
        ...
        for(int i=0; i<mKeys.length; i++) {
            ...
            int sel = 0; for(int j=0; j<mapOptions.length; j++) { if(mapOptions[j].startsWith(cur)) sel = j; } sp.setSelection(sel);
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        Button btnLang = new Button(this); btnLang.setText(isVi ? "🇻🇳 TIẾNG VIỆT" : "🇺🇸 ENGLISH"); btnLang.setTextColor(Color.WHITE); btnLang.setBackground(getRounded("#2E7D32", 20f)); btnLang.setOnClickListener(v -> { prefs.edit().putBoolean("lang_vi", !isVi).apply(); recreate(); });
        headerRow.addView(title); headerRow.addView(btnLang); main.addView(headerRow);
        
        if (!Settings.canDrawOverlays(this)) { Button btnReq = new Button(this); btnReq.setText(T("⚠️ CẤP QUYỀN", "⚠️ CẤP QUYỀN LỚP PHỦ")); btnReq.setBackground(getRounded("#D32F2F", 25f)); btnReq.setTextColor(Color.WHITE); btnReq.setOnClickListener(v -> startActivity(new Intent(Settings.ACTION_MANAGE_OVERLAY_PERMISSION, Uri.parse("package:" + getPackageName())))); main.addView(btnReq); }

        navMain = new LinearLayout(this); navMain.setOrientation(LinearLayout.HORIZONTAL); navMain.setPadding(0, 0, 0, 40);
        Button btnNavCond = new Button(this); btnNavCond.setText(T("CONDITIONS", "ĐIỀU KIỆN")); btnNavCond.setLayoutParams(new LinearLayout.LayoutParams(0, -2, 1f));
        Button btnNavMacs = new Button(this); btnNavMacs.setText("MACROS"); btnNavMacs.setLayoutParams(new LinearLayout.LayoutParams(0, -2, 1f));
        navMain.addView(btnNavCond); navMain.addView(btnNavMacs); main.addView(navMain);
        ...
        main.addView(pageDesign); main.addView(pageConditions); main.addView(pageMacros);
        ...
        btnDesign.setOnClickListener(v -> { if(pageDesign.getVisibility() == View.VISIBLE) { pageDesign.setVisibility(View.GONE); navMain.setVisibility(View.VISIBLE); pageConditions.setVisibility(View.VISIBLE); fab.setVisibility(View.VISIBLE); btnDesign.setText("⚙️"); btnDesign.setBackground(getRounded("#333333", 100f)); refreshPreview(); } else { currentMainTab = 0; pageConditions.setVisibility(View.GONE); pageMacros.setVisibility(View.GONE); navMain.setVisibility(View.GONE); pageDesign.setVisibility(View.VISIBLE); fab.setVisibility(View.GONE); btnDesign.setText("⬅"); btnDesign.setBackground(getRounded("#D32F2F", 100f)); refreshPreview(); } });

        bottomBar.addView(spacer); bottomBar.addView(fab); bottomBar.addView(btnDesign);
        rootLayout.addView(bottomBar);

        btnNavCond.setOnClickListener(v->{currentMainTab=1; refreshPreview(); pageConditions.setVisibility(View.VISIBLE); pageMacros.setVisibility(View.GONE); fab.setVisibility(View.VISIBLE); btnNavCond.setBackground(getRounded("#222222", 20f)); btnNavCond.setTextColor(Color.parseColor("#00E5FF")); btnNavMacs.setBackgroundColor(Color.TRANSPARENT); btnNavMacs.setTextColor(Color.GRAY); renderRulesList();}); 
        btnNavMacs.setOnClickListener(v->{currentMainTab=4; refreshPreview(); pageConditions.setVisibility(View.GONE); pageMacros.setVisibility(View.VISIBLE); fab.setVisibility(View.GONE); btnNavMacs.setBackground(getRounded("#222222", 20f)); btnNavMacs.setTextColor(Color.parseColor("#00E5FF")); btnNavCond.setBackgroundColor(Color.TRANSPARENT); btnNavCond.setTextColor(Color.GRAY);}); 
        btnNavCond.performClick(); setContentView(rootLayout);
    }
        ...
        for (int c = 0; c < ALL_COMP_KEYS.length; c++) {
            for (int g = 0; g < C_GESTURES.length; g++) {
                ...
                if (!action.equals("NONE")) {
                    ...
                    TextView tCond = new TextView(this); tCond.setText(ALL_COMP_NAMES[c] + "\n➔ " + C_GESTURE_NAMES[g]); tCond.setTextColor(Color.parseColor("#BBBBBB")); card.addView(tCond);
                    ...
                    tAct.setText(actName.toString().isEmpty() ? "Lỗi" : actName.toString()); tAct.setTextColor(Color.parseColor("#00E5FF")); card.addView(tAct);
                    ...
                    btnEdit.setOnClickListener(v -> openRuleBuilderDialog(key, c, g, ""));
                    ...
                    btnCopy.setOnClickListener(v -> openRuleBuilderDialog(null, c, g, action)); 
    ...
    private void openRuleBuilderDialog(String editKey, int preComp, int preGes, String copyActs) {
        Dialog d = new Dialog(this, android.R.style.Theme_DeviceDefault_NoActionBar_Fullscreen);
        ...
        TextView tvC = new TextView(this); tvC.setText("1. CHỌN VÙNG"); tvC.setTextColor(Color.parseColor("#E91E63")); content.addView(tvC);
        Spinner spComp = createSpinner(); spComp.setAdapter(new ArrayAdapter<>(this, android.R.layout.simple_spinner_dropdown_item, ALL_COMP_NAMES)); spComp.setSelection(preComp != -1 ? preComp : 0); content.addView(spComp);
        
        TextView tvG = new TextView(this); tvG.setText("\n2. CHỌN CỬ CHỈ"); tvG.setTextColor(Color.parseColor("#E91E63")); content.addView(tvG);
        ArrayList<CheckBox> gestureBoxes = new ArrayList<>();
        for (int i=0; i<C_GESTURES.length; i++) { CheckBox cb = new CheckBox(this); cb.setText(C_GESTURE_NAMES[i]); cb.setTextColor(Color.WHITE); if(preGes == i) cb.setChecked(true); gestureBoxes.add(cb); content.addView(cb); }
        
        TextView tvA = new TextView(this); tvA.setText("\n3. CHỌN ACTION"); tvA.setTextColor(Color.parseColor("#00E5FF")); content.addView(tvA);
        ArrayList<CheckBox> actionBoxes = new ArrayList<>();
        String savedActs = editKey != null ? prefs.getString(editKey, "") : copyActs; String[] savedArray = savedActs.split(",");
        for (int i=1; i<ACT_LABS.length; i++) { CheckBox cbAct = new CheckBox(this); cbAct.setText(ACT_LABS[i]); cbAct.setTextColor(Color.WHITE); boolean isChecked = false; for(String sa : savedArray) { if(sa.trim().equals(ACT_KEYS[i])) { isChecked = true; break; } } cbAct.setChecked(isChecked); actionBoxes.add(cbAct); content.addView(cbAct); }
        ...
        bSave.setOnClickListener(v -> {
            ...
            if(acts.isEmpty()) { Toast.makeText(this, "Chọn ít nhất 1 Action!", Toast.LENGTH_SHORT).show(); return; }
            String joinedActions = TextUtils.join(",", acts); String prefix = currentGesTab == 0 ? "lock_" : "home_"; String compKey = ALL_COMP_KEYS[spComp.getSelectedItemPosition()]; boolean hasChecked = false;
            if(editKey != null && preGes != -1) prefs.edit().putString(editKey, "NONE").apply(); 
            for(int i=0; i<gestureBoxes.size(); i++) { if(gestureBoxes.get(i).isChecked()) { hasChecked = true; prefs.edit().putString(prefix + compKey + "_" + C_GESTURES[i], joinedActions).apply(); } }
            if(!hasChecked) { Toast.makeText(this, "Chọn ít nhất 1 Trigger!", Toast.LENGTH_SHORT).show(); return; }
            renderRulesList(); d.dismiss();
        });
        d.setContentView(root); d.show();
    }
    ...
    private void buildMacrosSpace() {
        ...
        for(int i=1; i<=5; i++) { LinearLayout box = new LinearLayout(this); box.setOrientation(LinearLayout.VERTICAL); box.setPadding(30,10,30,30); box.addView(createInput("Tên gợi nhớ Macro", "macro_"+i+"_name")); box.addView(createInput("Danh sách Services (com.pkg/.Svc)", "macro_"+i+"_svcs")); pageMacros.addView(createDrawer("⚡ MACRO " + i, box)); }
    }

    private void buildDesignSpace() {
        LinearLayout secSys = new LinearLayout(this); secSys.setOrientation(LinearLayout.VERTICAL); secSys.addView(createSectionTitle("HÀNH VI HỆ THỐNG")); 
        CheckBox cbKbd = new CheckBox(this); cbKbd.setText("Tự ẩn khi hiện Bàn Phím"); cbKbd.setTextColor(Color.WHITE); cbKbd.setChecked(prefs.getBoolean("avoid_kbd", true)); cbKbd.setOnCheckedChangeListener((v,c) -> prefs.edit().putBoolean("avoid_kbd", c).apply()); secSys.addView(cbKbd); 
        ...
        Button btnPickApps = new Button(this); btnPickApps.setText("📱 CHỌN APP"); btnPickApps.setTextColor(Color.BLACK); btnPickApps.setBackground(getRounded("#00E5FF", 20f)); btnPickApps.setOnClickListener(v -> showAppPickerDialog());
        lockRow.addView(etLock); lockRow.addView(btnPickApps); secSys.addView(lockRow);
    ...
    private void renderSliders() { 
        ...
        if(designTabState == 3) { 
            ...
            String[] bKeys = designTabState == 2 ? M_BARS : BARS; String[] bNames = designTabState == 2 ? M_BAR_NAMES : BAR_NAMES;
            ...
            if(designTabState == 2) {
                ...
                btnTestM.setOnClickListener(v->{ Intent i = new Intent("com.manhmoc.edgebar.TOGGLE_MORSE"); sendBroadcast(i); });
                ...
                designSliderContainer.addView(createInput("Mật khẩu Master (VD: 12345)", "morse_master_pass"));
                ...
                designSliderContainer.addView(createSlider("Độ mờ màn chắn Morse", "morse_bg_alpha", 255, 180));
