...
public class MainActivity extends Activity {
    ...
    private void buildIntentsSpace() { for (int i=1;i<=15;i++) { LinearLayout sInt = new LinearLayout(this); sInt.setOrientation(LinearLayout.VERTICAL); sInt.setPadding(30,10,30,30); sInt.addView(createInput(T("Alias (e.g., Open FB)", "Tên gợi nhớ (VD: Mở Facebook)"), "intent_"+i+"_name")); sInt.addView(createInput("Action", "i"+i+"_act")); sInt.addView(createInput("Package", "i"+i+"_pkg")); sInt.addView(createInput("Class Name", "i"+i+"_cls")); sInt.addView(createInput("Data URI (acc://pkg/.cls)", "i"+i+"_data")); sInt.addView(createInput("Categories", "i"+i+"_cat")); sInt.addView(createInput("Flags", "i"+i+"_flags")); CheckBox cb = new CheckBox(this); cb.setText(T("Send as Broadcast", "Gửi dạng Broadcast")); cb.setTextColor(Color.WHITE); cb.setChecked(prefs.getBoolean("i"+i+"_br", true)); final int idx=i; cb.setOnCheckedChangeListener((v,c)->prefs.edit().putBoolean("i"+idx+"_br",c).apply()); sInt.addView(cb); pageIntents.addView(createDrawer("⚙ Intent Slot "+i, sInt)); } }
    private void buildMacrosSpace() { pageMacros.addView(createSectionTitle(T("ACCESSIBILITY MACROS", "ĐÓNG CẮT TRỢ NĂNG HÀNG LOẠT"))); TextView macDesc = new TextView(this); macDesc.setText(T("Format: com.pkg/.Service, com.pkg2/.Service", "Nhập tên Package/Class ngăn cách bởi dấu phẩy.\nVD: com.llamalab.macrodroid/.AccessibilitySvc")); macDesc.setTextColor(Color.GRAY); macDesc.setPadding(0,0,0,30); pageMacros.addView(macDesc); for(int i=1;i<=5;i++) { LinearLayout box = new LinearLayout(this); box.setOrientation(LinearLayout.VERTICAL); box.setPadding(30,10,30,30); box.addView(createInput(T("Macro Name", "Tên gợi nhớ Macro"), "macro_"+i+"_name")); box.addView(createInput(T("Target Services", "Danh sách Services"), "macro_"+i+"_svcs")); pageMacros.addView(createDrawer("⚡ MACRO "+i, box)); } }
    private void buildTilesSpace() { pageTiles.addView(createSectionTitle(T("15 CUSTOM QS TILES", "TÙY CHỈNH 15 NÚT QUICK SETTINGS"))); for(int i=1;i<=15;i++) { LinearLayout content = new LinearLayout(this); content.setOrientation(LinearLayout.VERTICAL); content.setPadding(30,10,30,30); LinearLayout row = new LinearLayout(this); row.setOrientation(LinearLayout.HORIZONTAL); row.setGravity(Gravity.CENTER_VERTICAL); TextView tv = new TextView(this); tv.setText(T("Action: ", "Hành động: ")); tv.setTextColor(Color.parseColor("#E91E63")); tv.setLayoutParams(new LinearLayout.LayoutParams(0,-2,1f)); Spinner s = createSpinner(); s.setAdapter(new ArrayAdapter<>(this, android.R.layout.simple_spinner_dropdown_item, ACT_LABS)); String v = prefs.getString("tile_"+i+"_act","NONE"); for(int j=0;j<ACT_KEYS.length;j++) if(ACT_KEYS[j].equals(v)) s.setSelection(j); final int idx=i; s.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener(){public void onItemSelected(AdapterView<?> p, View v, int pos, long id){prefs.edit().putString("tile_"+idx+"_act",ACT_KEYS[pos]).apply();}public void onNothingSelected(AdapterView<?> p){}}); s.setLayoutParams(new LinearLayout.LayoutParams(0,-2,1.5f)); row.addView(tv); row.addView(s); content.addView(row); CheckBox cbShow = new CheckBox(this); cbShow.setText(T("Enable this Tile", "Bật Tile này lên Hệ thống (Quick Settings)")); cbShow.setTextColor(Color.parseColor("#4CAF50")); cbShow.setPadding(0,20,0,0); ComponentName comp = new ComponentName(this, "com.manhmoc.edgebar.Tile" + i); int state = getPackageManager().getComponentEnabledSetting(comp); cbShow.setChecked(state == PackageManager.COMPONENT_ENABLED_STATE_ENABLED); cbShow.setOnCheckedChangeListener((vBtn, isChecked) -> { getPackageManager().setComponentEnabledSetting(comp, isChecked ? PackageManager.COMPONENT_ENABLED_STATE_ENABLED : PackageManager.COMPONENT_ENABLED_STATE_DISABLED, PackageManager.DONT_KILL_APP); Toast.makeText(this, isChecked ? T("Tile unlocked!", "Đã MỞ KHÓA Tile "+idx) : T("Tile hidden!", "Đã ẨN Tile "+idx), Toast.LENGTH_LONG).show(); }); content.addView(cbShow); pageTiles.addView(createDrawer("🔲 QS Tile "+i, content)); } }
    ...
    private void buildDesignSpace() {
        ...
        LinearLayout secSys = new LinearLayout(this); secSys.setOrientation(LinearLayout.VERTICAL); secSys.addView(createSectionTitle(T("SYSTEM BEHAVIOR", "HÀNH VI HỆ THỐNG"))); CheckBox cbKbd = new CheckBox(this); cbKbd.setText(T("Auto-hide on Keyboard", "Tự ẩn khi hiện Bàn Phím")); cbKbd.setTextColor(Color.WHITE); cbKbd.setChecked(prefs.getBoolean("avoid_kbd", true)); cbKbd.setOnCheckedChangeListener((v,c) -> prefs.edit().putBoolean("avoid_kbd", c).apply()); secSys.addView(cbKbd); secSys.addView(createSectionTitle("BLACKLIST (Hide Overlay)")); secSys.addView(createInput("Packages (com.ex.app)", "blacklist")); secSys.addView(createSectionTitle("LOCKLIST (Morse AppLock)")); secSys.addView(createInput("Packages (com.zing.zalo, com.mbmobile)", "locklist")); pageDesign.addView(wrapCard(secSys));
    ...
    private void showPremiumDialog() { 
        String t = T("ADB COMMANDS:\nadb shell pm grant com.manhmoc.edgebar android.permission.WRITE_SECURE_SETTINGS\nadb shell appops set com.manhmoc.edgebar SYSTEM_ALERT_WINDOW allow", 
                     "🔧 LỆNH ADB CỐT LÕI (Cấp 1 lần):\n\n1. Quyền ghi Cài đặt bảo mật:\nadb shell pm grant com.manhmoc.edgebar android.permission.WRITE_SECURE_SETTINGS\n\n2. Quyền vẽ Lớp phủ (Tàng hình AppOps):\nadb shell appops set com.manhmoc.edgebar SYSTEM_ALERT_WINDOW allow\n\n🚀 V19.12.3.5 THE FINAL MASTERPIECE:\n- Tổng hợp 16 phiên bản, sạch lỗi compile\n- Grid UI, Multi-Action, Rule Engine, Morse AppLock, 15 QS Tiles, 5 Macros\n- Live Preview, Phantom Overlay, Independent Moon Core\n- Zero-day exploit patched, fully stable");
