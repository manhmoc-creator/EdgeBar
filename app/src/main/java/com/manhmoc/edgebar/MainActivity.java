...
public class MainActivity extends Activity {
    ...
    private String[] BARS = {"r", "l", "t_r", "t_l", "t_c", "b_c", "mid_t", "mid_b"}; 
    ...
    private String[] ALL_COMP_KEYS = {"r", "l", "t_r", "t_l", "t_c", "b_c", "mid_t", "mid_b", "corner_br", "corner_bl", "corner_tr", "corner_tl"}; 
    private String[] C_GESTURES = {"tap", "dtap", "long", "up", "down", "left", "right", "up_hold", "down_hold", "left_hold", "right_hold", "diag", "diag_hold"}; 

    private String[] BAR_NAMES; private String[] CORNER_NAMES; private String[] COLOR_NAMES; private String[] ALL_COMP_NAMES; private String[] C_GESTURE_NAMES;
    ...
    private void reloadActionLabels() {
        String[] bK = {"NONE", "BACK", "HOME", "RECENTS", "SCREEN_OFF", "FLASH", "POWER_DIALOG", "VOLUME", "SCREENSHOT", "CAMERA", "NOTIFICATIONS", "TOGGLE_ACC", "TOGGLE_OVERLAY", "TOGGLE_MORSE", "YTDL_DOWNLOAD"}; 
        String[] bL = {T("None","Không có"), T("Back","Quay lại"), T("Home","Màn hình chính"), T("Recents","Đa nhiệm"), T("Screen Off","Tắt màn hình"), T("Flashlight","Đèn pin"), T("Power Menu","Menu Nguồn"), T("Volume Panel","Menu Âm Lượng"), T("Screenshot","Chụp ảnh màn hình"), "Camera", T("Notifications","Mở Thông Báo"), T("Toggle Acc","Bật/Tắt Trợ Năng"), T("Toggle Overlay","Bật/Tắt Lớp Phủ"), T("Toggle MORSE","Bật/Tắt Không gian MORSE"), "Tải bằng YTDLnis"};
    ...
    // V19.12.2.0: PHỤC HỒI DÂY THẦN KINH BACKUP/RESTORE I/O
        ...
        if(res == RESULT_OK && data != null && data.getData() != null) { 
            try { 
                if(req == 101) { 
                    ...
                    Toast.makeText(this, T("Backup Saved!", "Đã Lưu Cấu Hình!"), Toast.LENGTH_SHORT).show(); 
                } else if(req == 102) { 
                    ...
                    SharedPreferences.Editor ed = prefs.edit(); Iterator<String> k = j.keys(); 
                    while(k.hasNext()) { 
                    ...
                    ed.apply(); Toast.makeText(this, T("Restored!", "Đã Khôi Phục!"), Toast.LENGTH_LONG).show(); recreate();
                } 
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        BAR_NAMES = new String[]{T("Right bottom", "Đáy phải"), T("Left bottom", "Đáy trái"), T("Right top", "Cạnh Phải"), T("Left top", "Cạnh Trái"), T("Center top", "Đỉnh giữa"), T("Bottom center", "Đáy giữa"), T("Top half", "Giữa Trên"), T("Bottom half", "Giữa Dưới")}; 
        ...
        COLOR_NAMES = new String[]{"Trắng Tinh Khiết", "Neon", "Cyberpunk", "Lava", "Ocean", "Matrix", "Sunset", "Google", "Aurora", "Abyss", "Forest", "Flame", "Midnight", "Tropical", "Candy"};
        ALL_COMP_NAMES = new String[]{T("Right bottom", "Thanh Đáy Phải"), T("Left bottom", "Thanh Đáy Trái"), T("Right top", "Thanh Cạnh Phải"), T("Left top", "Thanh Cạnh Trái"), T("Center top", "Thanh Đỉnh Giữa"), T("Bottom center", "Thanh Đáy Giữa"), T("Top half", "Thanh Giữa Trên"), T("Bottom half", "Thanh Giữa Dưới"), T("Bottom right corner", "Góc Viền Đáy Phải"), T("Bottom left corner", "Góc Viền Đáy Trái"), T("Top right corner", "Góc Viền Đỉnh Phải"), T("Top left corner", "Góc Viền Đỉnh Trái")};
        C_GESTURE_NAMES = new String[]{T("Tap","1 Chạm"), T("Double Tap","2 Chạm"), T("Long Press","Nhấn Giữ"), T("Swipe Up","Vuốt Lên"), T("Swipe Down","Vuốt Xuống"), T("Swipe Left","Vuốt Trái"), T("Swipe Right","Vuốt Phải"), T("Up + Hold","Vuốt Lên + Giữ"), T("Down + Hold","Vuốt Xuống + Giữ"), T("Left + Hold","Vuốt Trái + Giữ"), T("Right + Hold","Vuốt Phải + Giữ"), T("Diagonal","Vuốt Chéo"), T("Diag + Hold","Vuốt Chéo + Giữ")};
        ...
        Button btnUpdate = createCircleBtn("↻", "#333333"); btnUpdate.setTextSize(22); btnUpdate.setOnClickListener(v -> { Intent i = new Intent(Intent.ACTION_VIEW); i.setData(Uri.parse("https://github.com/manhmoc-creator/EdgeBar/actions")); startActivity(i); });
        ...
        LinearLayout.LayoutParams fLp = new LinearLayout.LayoutParams(-2, -1); fLp.setMargins(10,0,10,0); fab.setLayoutParams(fLp); fab.setPadding(40,0,40,0); fab.setOnClickListener(v -> openRuleBuilderDialog(null, -1, -1));
        Button btnDesign = createCircleBtn("⚙️", "#333333"); btnDesign.setTag("btnDesign"); btnDesign.setOnClickListener(v -> { if(pageDesign.getVisibility() == View.VISIBLE) { closeDesignSpace(); btnDesign.setText("⚙️"); btnDesign.setBackground(getRounded("#333333", 100f)); } else { openDesignSpace(); btnDesign.setText("⬅"); btnDesign.setBackground(getRounded("#D32F2F", 100f)); } });

        bottomBar.addView(btnUpdate); bottomBar.addView(btnPremium); bottomBar.addView(spacer); bottomBar.addView(fab); bottomBar.addView(btnDesign); rootLayout.addView(bottomBar);
    ...
    private void buildConditionsSpace() {
        ...
        btnLock = createTabBtn("LOCKSCREEN"); btnHome = createTabBtn("HOMESCREEN"); btnMorse = createTabBtn("MORSE"); 
        LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(0, -2, 1f); p.setMargins(0,0,10,0); btnLock.setLayoutParams(p); btnHome.setLayoutParams(p); btnMorse.setLayoutParams(new LinearLayout.LayoutParams(0, -2, 1f));
        tabContainer.addView(btnLock); tabContainer.addView(btnHome); tabContainer.addView(btnMorse); pageConditions.addView(tabContainer);
        listRules = new LinearLayout(this); listRules.setOrientation(LinearLayout.VERTICAL); pageConditions.addView(listRules);
        
        View.OnClickListener tC = v -> { currentGesTab = (v==btnLock)?0:(v==btnHome)?1:2; refreshPreview(); btnLock.setBackground(getRounded(v==btnLock?"#00E5FF":"#222222", 20f)); btnLock.setTextColor(v==btnLock?Color.BLACK:Color.WHITE); btnHome.setBackground(getRounded(v==btnHome?"#00E5FF":"#222222", 20f)); btnHome.setTextColor(v==btnHome?Color.BLACK:Color.WHITE); btnMorse.setBackground(getRounded(v==btnMorse?"#00E5FF":"#222222", 20f)); btnMorse.setTextColor(v==btnMorse?Color.BLACK:Color.WHITE); renderRulesList(); };
        btnLock.setOnClickListener(tC); btnHome.setOnClickListener(tC); btnMorse.setOnClickListener(tC); btnLock.performClick();
    }
        ...
        for (int c = 0; c < ALL_COMP_KEYS.length; c++) {
            for (int g = 0; g < C_GESTURES.length; g++) {
                ...
                if (!action.equals("NONE")) {
                    ...
                    card.setOnLongClickListener(v -> { new AlertDialog.Builder(this).setTitle(T("Delete Rule?","Xóa Quy tắc?")).setPositiveButton("XÓA", (d,w)->{prefs.edit().putString(key, "NONE").apply(); renderRulesList();}).setNegativeButton("HỦY", null).show(); return true; });
    ...
    private View buildRuleEditor(Dialog dialog, String editKey, int preComp, int preGes) {
        ...
        LinearLayout.LayoutParams tabLp = new LinearLayout.LayoutParams(0,-2,1f); tabLp.setMargins(10,0,10,0); bTrig.setLayoutParams(tabLp); bAct.setLayoutParams(tabLp); bOpt.setLayoutParams(tabLp);
        ...
        TextView tvA = new TextView(this); tvA.setText("CHỌN HÀNH ĐỘNG THỰC THI (Được chọn nhiều - Bắn liên thanh)"); tvA.setTextColor(Color.parseColor("#00E5FF")); tvA.setPadding(0,0,0,20); vAct.addView(tvA);
        ...
        cbVib.setText("Bật Rung (Haptic Feedback)"); cbVib.setTextColor(Color.WHITE); cbVib.setChecked(editKey == null || prefs.getBoolean(editKey+"_vib", true)); vOpt.addView(cbVib);
        cbAnim.setText("Bật Hiệu ứng Ánh sáng (Animation)"); cbAnim.setTextColor(Color.WHITE); cbAnim.setChecked(editKey == null || prefs.getBoolean(editKey+"_anim", true)); vOpt.addView(cbAnim);
        ...
        bSave.setOnClickListener(v -> {
            ...
            String joinedActions = TextUtils.join(",", acts); String prefix = currentGesTab == 0 ? "lock_" : (currentGesTab == 1 ? "home_" : "morse_"); String compKey = ALL_COMP_KEYS[selectedComp[0]]; boolean hasChecked = false;
    ...
    private void buildMacrosSpace() {
        pageMacros.addView(createSectionTitle("ĐÓNG CẮT TRỢ NĂNG HÀNG LOẠT")); TextView macDesc = new TextView(this); macDesc.setText("Nhập tên Package/Class ngăn cách bởi dấu phẩy.\nVD: com.llamalab.macrodroid/.AccessibilitySvc"); macDesc.setTextColor(Color.GRAY); macDesc.setPadding(0,0,0,30); pageMacros.addView(macDesc);
        for(int i=1; i<=5; i++) { LinearLayout box = new LinearLayout(this); box.setOrientation(LinearLayout.VERTICAL); box.setPadding(30,10,30,30); box.addView(createInput("Tên gợi nhớ Macro", "macro_"+i+"_name")); box.addView(createInput("Danh sách Services", "macro_"+i+"_svcs")); pageMacros.addView(createDrawer("⚡ MACRO " + i, box)); }
    }
    ...
    private void buildDesignSpace() {
        pageDesign.addView(createSectionTitle("KHU VỰC SAO LƯU"));
        ...
        pageDesign.addView(createSectionTitle("THIẾT KẾ CỐT LÕI (MÀU/KÍCH THƯỚC)"));
        LinearLayout toggleRow = new LinearLayout(this); toggleRow.setOrientation(LinearLayout.HORIZONTAL);
        btnEditLock = new Button(this); btnEditLock.setText("LOCK"); btnEditHome = new Button(this); btnEditHome.setText("HOME"); btnMorse = new Button(this); btnMorse.setText("MORSE"); btnEditAnim = new Button(this); btnEditAnim.setText("ANIMA"); 
        LinearLayout.LayoutParams tLp = new LinearLayout.LayoutParams(0,-2,1f); tLp.setMargins(5,0,5,0); btnEditLock.setLayoutParams(tLp); btnEditHome.setLayoutParams(tLp); btnMorse.setLayoutParams(tLp); btnEditAnim.setLayoutParams(tLp);
        ...
        View.OnClickListener dClick = v -> { designTabState = (v==btnEditLock)?0:(v==btnEditHome)?1:(v==btnMorse)?2:3; refreshPreview(); btnEditLock.setBackground(getRounded(v==btnEditLock?"#00E5FF":"#222222", 20f)); btnEditLock.setTextColor(v==btnEditLock?Color.BLACK:Color.WHITE); btnEditHome.setBackground(getRounded(v==btnEditHome?"#00E5FF":"#222222", 20f)); btnEditHome.setTextColor(v==btnEditHome?Color.BLACK:Color.WHITE); btnMorse.setBackground(getRounded(v==btnMorse?"#00E5FF":"#222222", 20f)); btnMorse.setTextColor(v==btnMorse?Color.BLACK:Color.WHITE); btnEditAnim.setBackground(getRounded(v==btnEditAnim?"#00E5FF":"#222222", 20f)); btnEditAnim.setTextColor(v==btnEditAnim?Color.BLACK:Color.WHITE); renderSliders(); };
        btnEditLock.setOnClickListener(dClick); btnEditHome.setOnClickListener(dClick); btnMorse.setOnClickListener(dClick); btnEditAnim.setOnClickListener(dClick);
        toggleRow.addView(btnEditLock); toggleRow.addView(btnEditHome); toggleRow.addView(btnMorse); toggleRow.addView(btnEditAnim); pageDesign.addView(toggleRow); pageDesign.addView(designSliderContainer); btnEditLock.performClick();
    }

    private void showPremiumDialog() { String t = "🔧 LỆNH ADB CỐT LÕI (Cấp 1 lần):\n\n1. Quyền ghi Cài đặt bảo mật (Bắt buộc cho Trợ Năng):\nadb shell pm grant com.manhmoc.edgebar android.permission.WRITE_SECURE_SETTINGS\n\n2. Quyền vẽ Lớp phủ (Tàng hình AppOps):\nadb shell appops set com.manhmoc.edgebar SYSTEM_ALERT_WINDOW allow\n\n🚀 V19.12.2.0 MORSE ENGINE:\nKhông gian bảo mật tuyệt đối. Phủ màn đen vô hình chặn mọi thao tác, chỉ người tạo ra quy tắc gõ Morse mới biết cách xuyên qua!"; ScrollView sv = new ScrollView(this); sv.setPadding(50,50,50,50); TextView tv = new TextView(this); tv.setText(t); tv.setTextColor(Color.WHITE); tv.setTextSize(15f); tv.setLineSpacing(0, 1.3f); sv.addView(tv); new AlertDialog.Builder(this, android.R.style.Theme_DeviceDefault_Dialog_Alert).setTitle("👑 PREMIUM ARCHITECT INFO").setView(sv).setPositiveButton("OK", null).show(); }
    ...
    private void renderSliders() { designSliderContainer.removeAllViews(); if(designTabState == 3) { 
        ...
        if (designTabState == 2) { // MORSE MASTER SWITCHES
            CheckBox cbMorseM = new CheckBox(this); cbMorseM.setText("BẬT LỚP PHỦ MORSE NGAY LẬP TỨC (Khoá màn hình chính)"); cbMorseM.setTextColor(Color.parseColor("#FF00FF")); cbMorseM.setChecked(prefs.getBoolean("morse_on", false)); cbMorseM.setOnCheckedChangeListener((v,c) -> { prefs.edit().putBoolean("morse_on", c).apply(); sendBroadcast(new Intent("com.manhmoc.edgebar.SYNC_STATE")); }); designSliderContainer.addView(cbMorseM);
            designSliderContainer.addView(createSlider("Độ mờ màn chắn đen MORSE (0=Trong suốt, 255=Đen xì)", "morse_bg_alpha", 255, 200));
        }

        designSliderContainer.addView(createSectionTitle("8 EDGE BARS CHÍNH PHỤ"));
        for(int i=0; i< 8; i++) { LinearLayout drawerContent = new LinearLayout(this); drawerContent.setOrientation(LinearLayout.VERTICAL); drawerContent.setPadding(30,10,30,30); CheckBox cb = new CheckBox(this); cb.setText("BẬT: " + BAR_NAMES[i]); cb.setTextColor(Color.parseColor("#4CAF50")); cb.setChecked(prefs.getBoolean(prefix+BARS[i]+"_en", false)); final int idx = i; cb.setOnCheckedChangeListener((v,c) -> prefs.edit().putBoolean(prefix+BARS[idx]+"_en", c).apply()); drawerContent.addView(cb); drawerContent.addView(createComboDropdown("Chế độ Cảm ứng", prefix+BARS[i]+"_pri_mode", new String[]{"Ưu tiên Edge Bar (Khoá cứng)", "Nhường OS (Xuyên thấu)"}, 0)); drawerContent.addView(createSlider("Độ trong suốt", prefix+BARS[i]+"_alpha", 255, 50)); drawerContent.addView(createSlider("Chiều ngang", prefix+BARS[i]+"_w", 3000, 300)); drawerContent.addView(createSlider("Chiều dọc", prefix+BARS[i]+"_h", 3000, 60)); drawerContent.addView(createSlider("Toạ độ X", prefix+BARS[i]+"_x", 1000, 0)); drawerContent.addView(createSlider("Toạ độ Y", prefix+BARS[i]+"_y", 1000, 0)); designSliderContainer.addView(createDrawer(BAR_NAMES[i], drawerContent)); } 
        designSliderContainer.addView(createSectionTitle("4 FRAME CORNERS (GÓC VIỀN)"));
