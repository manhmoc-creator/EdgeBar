...
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        // Tab Hệ Sinh Thái Header
        LinearLayout ecoTabRow = new LinearLayout(this); ecoTabRow.setOrientation(LinearLayout.HORIZONTAL); ecoTabRow.setPadding(0,0,0,20);
        Button eI = createTabBtn("🔴 INTENTS"); Button eQ = createTabBtn("🟢 QS TILES"); Button eM = createTabBtn("🔵 MACROS");
        LinearLayout.LayoutParams tp = new LinearLayout.LayoutParams(0,-2,1f); tp.setMargins(5,0,5,0); eI.setLayoutParams(tp); eQ.setLayoutParams(tp); eM.setLayoutParams(tp);
        ecoTabRow.addView(eI); ecoTabRow.addView(eQ); ecoTabRow.addView(eM); pageEcosystem.addView(ecoTabRow);
        listEco = new LinearLayout(this); listEco.setOrientation(LinearLayout.VERTICAL); pageEcosystem.addView(listEco);
        
        View.OnClickListener ecoClick = v -> {
            eI.setBackground(getRounded(v==eI?"#E53935":"#222222", 15f)); eI.setTextColor(Color.WHITE); 
            eQ.setBackground(getRounded(v==eQ?"#43A047":"#222222", 15f)); eQ.setTextColor(Color.WHITE); 
            eM.setBackground(getRounded(v==eM?"#1E88E5":"#222222", 15f)); eM.setTextColor(Color.WHITE); 
            currentEcoTab = (v==eI) ? 0 : ((v==eQ) ? 1 : 2); renderEcosystemList();
        }; eI.setOnClickListener(ecoClick); eQ.setOnClickListener(ecoClick); eM.setOnClickListener(ecoClick); eI.performClick();
        ...
        // V19.12.2.12: Chức năng Key Mapper - Chỉ mở 1 Slot Trống
        fabI.setOnClickListener(v -> openNewEcoSlot("INTENT", 15)); 
        fabQ.setOnClickListener(v -> openNewEcoSlot("TILE", 15)); 
        fabM.setOnClickListener(v -> openNewEcoSlot("MACRO", 5));
    ...
    // V19.12.2.12: Layout 2 ô ngang cho Ecosystem
    private void renderEcosystemList() {
        ...
        if(currentEcoTab == 0) {
            for (int i = 1; i <= 15; i++) { String name = prefs.getString("intent_"+i+"_name", ""); if(!name.isEmpty()) { 
                ...
                currentRow.addView(createEcoCard("🔴 INTENT " + i, name, detail, "INTENT", i, "intent_"+i+"_on")); count++; 
            } }
        } else if (currentEcoTab == 1) {
            for (int i = 1; i <= 15; i++) { String act = prefs.getString("tile_"+i+"_act", "NONE"); if(!act.equals("NONE")) { 
                ...
                if(count % 2 == 0) { currentRow = new LinearLayout(this); currentRow.setOrientation(LinearLayout.HORIZONTAL); currentRow.setLayoutParams(new LinearLayout.LayoutParams(-1,-2)); listEco.addView(currentRow); }
                currentRow.addView(createEcoCard("🟢 QS TILE " + i, actName, "Sys: " + (getPackageManager().getComponentEnabledSetting(new ComponentName(this, "com.manhmoc.edgebar.Tile"+i)) == PackageManager.COMPONENT_ENABLED_STATE_ENABLED ? "Đã Bật" : "Đã Tắt"), "TILE", i, "tile_"+i+"_on")); count++; 
            } }
        } else {
            for (int i = 1; i <= 5; i++) { String name = prefs.getString("macro_"+i+"_name", ""); if(!name.isEmpty()) { 
                if(count % 2 == 0) { currentRow = new LinearLayout(this); currentRow.setOrientation(LinearLayout.HORIZONTAL); currentRow.setLayoutParams(new LinearLayout.LayoutParams(-1,-2)); listEco.addView(currentRow); }
                currentRow.addView(createEcoCard("🔵 MACRO " + i, name, "Svc: " + prefs.getString("macro_"+i+"_svcs", "Trống"), "MACRO", i, "macro_"+i+"_on")); count++; 
            } }
        }
        if(count % 2 != 0 && currentRow != null) { View dummy = new View(this); dummy.setLayoutParams(new LinearLayout.LayoutParams(0,1,1f)); currentRow.addView(dummy); }
        if(count == 0) { TextView empty = new TextView(this); empty.setText("Không gian trống.\nBấm viên thuốc bên dưới để tạo."); empty.setTextColor(Color.GRAY); empty.setGravity(Gravity.CENTER); empty.setPadding(0,100,0,0); listEco.addView(empty); }
    }

    private View createEcoCard(String title, String desc, String detail, String type, int id, String swKey) {
        LinearLayout card = new LinearLayout(this); card.setOrientation(LinearLayout.VERTICAL); card.setBackground(getRounded("#1E1E1E", 25f)); card.setPadding(35,35,35,35); LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(0,-2, 1f); lp.setMargins(15,15,15,15); card.setLayoutParams(lp);
        ...
        card.setOnClickListener(v -> openEcoEditor(type, id));
        card.setOnLongClickListener(v -> { new AlertDialog.Builder(this).setTitle("Xóa Mục này?").setPositiveButton("XÓA", (d,w)->{ if(type.equals("INTENT")) prefs.edit().putString("intent_"+id+"_name", "").apply(); else if(type.equals("MACRO")) prefs.edit().putString("macro_"+id+"_name", "").apply(); else prefs.edit().putString("tile_"+id+"_act", "NONE").apply(); renderEcosystemList(); reloadActionLabels(); }).setNegativeButton("HỦY", null).show(); return true; });
        return card;
    }

    // V19.12.2.12: Tính năng Key Mapper - Mở Không gian nhỏ cho 1 item
    private void openNewEcoSlot(String type, int max) {
        int slot = -1;
        for(int i=1; i<=max; i++) {
            if(type.equals("INTENT") && prefs.getString("intent_"+i+"_name", "").isEmpty()) { slot = i; break; }
            if(type.equals("MACRO") && prefs.getString("macro_"+i+"_name", "").isEmpty()) { slot = i; break; }
            if(type.equals("TILE") && prefs.getString("tile_"+i+"_act", "NONE").equals("NONE")) { slot = i; break; }
        }
        if(slot != -1) openEcoEditor(type, slot); else Toast.makeText(this, "Không gian " + type + " đã đầy!", Toast.LENGTH_SHORT).show();
    }

    private void openEcoEditor(String type, int id) {
        ...
        TextView title = new TextView(this); title.setText("CẤU HÌNH " + type + " SLOT " + id); title.setTextColor(Color.parseColor("#00E5FF")); title.setTextSize(20); title.setPadding(0,0,0,40); root.addView(title);
        ScrollView scroll = new ScrollView(this); scroll.setLayoutParams(new LinearLayout.LayoutParams(-1,0,1f)); LinearLayout content = new LinearLayout(this); content.setOrientation(LinearLayout.VERTICAL); scroll.addView(content); root.addView(scroll);
        
        if (type.equals("INTENT")) { content.addView(createInput("Tên gợi nhớ (VD: Mở Facebook)", "intent_"+id+"_name")); content.addView(createInput("Action", "intent_"+id+"_act")); content.addView(createInput("Package", "intent_"+id+"_pkg")); content.addView(createInput("Class Name", "intent_"+id+"_cls")); content.addView(createInput("Data URI (acc://pkg/.cls)", "intent_"+id+"_data")); content.addView(createInput("Categories", "intent_"+id+"_cat")); content.addView(createInput("Flags", "intent_"+id+"_flags")); CheckBox cb = new CheckBox(this); cb.setText("Gửi dạng Broadcast"); cb.setTextColor(Color.WHITE); cb.setChecked(prefs.getBoolean("intent_"+id+"_br", true)); cb.setOnCheckedChangeListener((v,c) -> prefs.edit().putBoolean("intent_"+id+"_br", c).apply()); content.addView(cb);
        } else if (type.equals("MACRO")) { content.addView(createInput("Tên gợi nhớ Macro", "macro_"+id+"_name")); content.addView(createInput("Nhập Packages/Classes (VD: com.a/.b, com.c/.d)", "macro_"+id+"_svcs"));
        } else if (type.equals("TILE")) { TextView tv = new TextView(this); tv.setText("Hành động Quick Settings: "); tv.setTextColor(Color.parseColor("#E91E63")); content.addView(tv); Spinner s = createSpinner(); s.setAdapter(new ArrayAdapter<>(this, android.R.layout.simple_spinner_dropdown_item, ACT_LABS)); String v = prefs.getString("tile_"+id+"_act","NONE"); for(int j=0;j<ACT_KEYS.length;j++) if(ACT_KEYS[j].equals(v)) s.setSelection(j); s.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener(){public void onItemSelected(AdapterView<?> p, View v, int pos, long idL){prefs.edit().putString("tile_"+id+"_act",ACT_KEYS[pos]).apply();}public void onNothingSelected(AdapterView<?> p){}}); content.addView(s); CheckBox cbShow = new CheckBox(this); cbShow.setText("Bật Tile lên Hệ thống Quick Settings"); cbShow.setTextColor(Color.parseColor("#4CAF50")); ComponentName comp = new ComponentName(this, "com.manhmoc.edgebar.Tile" + id); cbShow.setChecked(getPackageManager().getComponentEnabledSetting(comp) == PackageManager.COMPONENT_ENABLED_STATE_ENABLED); cbShow.setOnCheckedChangeListener((vBtn, isChecked) -> { getPackageManager().setComponentEnabledSetting(comp, isChecked ? PackageManager.COMPONENT_ENABLED_STATE_ENABLED : PackageManager.COMPONENT_ENABLED_STATE_DISABLED, PackageManager.DONT_KILL_APP); Toast.makeText(this, isChecked ? "Đã MỞ KHÓA Tile!" : "Đã ẨN Tile", Toast.LENGTH_SHORT).show(); }); content.addView(cbShow); }
        
        LinearLayout footer = new LinearLayout(this); footer.setOrientation(LinearLayout.HORIZONTAL); Button bCancel = new Button(this); bCancel.setText("HỦY"); bCancel.setBackground(getRounded("#333333", 20f)); bCancel.setTextColor(Color.WHITE); bCancel.setLayoutParams(new LinearLayout.LayoutParams(0,-2,1f)); Button bSave = new Button(this); bSave.setText("LƯU LẠI"); bSave.setBackground(getRounded("#4CAF50", 20f)); bSave.setTextColor(Color.WHITE); LinearLayout.LayoutParams slp = new LinearLayout.LayoutParams(0,-2,1f); slp.setMargins(20,0,0,0); bSave.setLayoutParams(slp); footer.addView(bCancel); footer.addView(bSave); root.addView(footer);
        bCancel.setOnClickListener(v -> d.dismiss()); bSave.setOnClickListener(v -> { reloadActionLabels(); renderEcosystemList(); d.dismiss(); }); d.show();
    }
    ...
    private void buildDesignSpace() {
        ...
        // V19.12.2.12: Hotfix YTDLnis dùng createChooser để Android xử lý mượt, tránh nháy giao diện
        ...
        btnYtdl.setOnClickListener(v -> {
            ...
            try { Intent intent = new Intent(Intent.ACTION_SEND); intent.setType("text/plain"); intent.putExtra(Intent.EXTRA_TEXT, q); startActivity(Intent.createChooser(intent, "Tìm bằng YTDLnis")); } catch(Exception e) { Toast.makeText(this, "Lỗi!", Toast.LENGTH_SHORT).show(); }
        }); 
    ...
    // Test Button cho Hơi thở để kiểm tra kích thước View
    private void testBreathAnimation() {
        ...
        WindowManager wm = (WindowManager) getSystemService(WINDOW_SERVICE); RecorderService.BreathView bv = new RecorderService.BreathView(this, prefs);
        ...
        if (shape == 1) { params.gravity = Gravity.TOP | Gravity.LEFT; params.x = bX; params.y = bY; bv.setOnClickListener(v -> { wm.removeView(bv); Toast.makeText(this, "Đã DỪNG thử!", Toast.LENGTH_SHORT).show(); }); } else { params.gravity = Gravity.TOP | Gravity.CENTER_HORIZONTAL; }
        ObjectAnimator anim = ObjectAnimator.ofFloat(bv, "animAlpha", 0.1f, 1f, 0.1f); anim.setDuration(prefs.getInt("breath_delay", 1500)); anim.setRepeatCount(ValueAnimator.INFINITE); anim.start(); wm.addView(bv, params);
        new Handler().postDelayed(() -> { try { wm.removeView(bv); anim.cancel(); } catch(Exception e){} }, 4000);
    }

    private void renderSliders() { designSliderContainer.removeAllViews(); if(designTabState == 2) { 
        // V19.12.2.12: Quy hoạch cả Viền và Hơi thở ghi âm vào chung tab ANIMA
        designSliderContainer.addView(createSectionTitle("✨ HIỆU ỨNG ÁNH SÁNG (VIỀN GỐC)"));
        ...
        designSliderContainer.addView(createSectionTitle("🎤 HƠI THỞ GHI ÂM (BREATH)"));
        Button btnTestBreath = new Button(this); btnTestBreath.setText("▶ THỬ ANIMATION HƠI THỞ"); btnTestBreath.setBackground(getRounded("#E91E63", 20f)); btnTestBreath.setTextColor(Color.WHITE); btnTestBreath.setPadding(0,30,0,30); LinearLayout.LayoutParams testLp2 = new LinearLayout.LayoutParams(-1,-2); testLp2.setMargins(0,20,0,20); btnTestBreath.setLayoutParams(testLp2); btnTestBreath.setOnClickListener(v -> testBreathAnimation()); designSliderContainer.addView(btnTestBreath);
        designSliderContainer.addView(createComboDropdown("Chất lượng Ghi âm (kbps)", "rec_kbps", new String[]{"64 kbps", "128 kbps", "256 kbps", "320 kbps (High)"}, 1));
        designSliderContainer.addView(createComboDropdown("Hình dáng Hơi Thở", "breath_shape", new String[]{"Dải Viền (Edge)", "Chấm Tròn Cảm Ứng (Dot)"}, 0));
