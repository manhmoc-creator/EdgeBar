...
public class MainActivity extends Activity {
    ...
    private void reloadActionLabels() {
        String[] bK = {"NONE", "BACK", "HOME", "RECENTS", "SCREEN_OFF", "FLASH", "POWER_DIALOG", "VOLUME", "SCREENSHOT", "CAMERA", "NOTIFICATIONS", "TOGGLE_ACC", "TOGGLE_OVERLAY", "YTDL_DOWNLOAD", "TOGGLE_MORSE"}; 
        String[] bL = {"Không có", "Quay lại", "Màn hình chính", "Đa nhiệm", "Tắt màn hình", "Đèn pin", "Menu Nguồn", "Menu Âm Lượng", "Chụp ảnh màn hình", "Camera", "Mở Thông Báo", "Bật/Tắt Trợ Năng", "Bật/Tắt Lớp Phủ", "Tải bằng YTDLnis", "Khóa App (Morse)"};
    ...
    private void renderSliders() { 
        ...
        if(designTabState == 3) { 
            ...
            // Đối với Morse chỉ hiển thị các thanh đặc biệt (đã được xử lý trong HomescreenService, ở đây giữ nguyên danh sách thanh)
            String[] bKeys = BARS; String[] bNames = BAR_NAMES;
            if(designTabState == 2) {
                // Morse UI settings
                Button btnRecord = new Button(this); btnRecord.setText("BẮT ĐẦU GHI MẬT KHẨU MORSE"); btnRecord.setBackground(getRounded("#E91E63", 20f)); btnRecord.setTextColor(Color.WHITE); btnRecord.setPadding(0,30,0,30); LinearLayout.LayoutParams recLp = new LinearLayout.LayoutParams(-1,-2); recLp.setMargins(0,0,0,20); btnRecord.setLayoutParams(recLp);
