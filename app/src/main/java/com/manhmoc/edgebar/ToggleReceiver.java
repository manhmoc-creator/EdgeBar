...
import android.content.BroadcastReceiver; import android.content.Context; import android.content.Intent; import android.content.SharedPreferences; import android.provider.Settings; import android.os.Build;
public class ToggleReceiver extends BroadcastReceiver {
    @Override public void onReceive(Context c, Intent i) {
        if ("com.manhmoc.edgebar.TOGGLE_LOCK".equals(i.getAction())) {
            ...
            SharedPreferences p = c.getSharedPreferences("EdgeBarPrefs", Context.MODE_PRIVATE); p.edit().putBoolean("morse_on", !p.getBoolean("morse_on", false)).apply(); c.sendBroadcast(new Intent("com.manhmoc.edgebar.SYNC_STATE"));
        } else if ("com.manhmoc.edgebar.TOGGLE_MACRO".equals(i.getAction())) {
            String list = i.getStringExtra("services");
            if(list != null && !list.isEmpty()) {
                String[] svcs = list.split(","); String cur = Settings.Secure.getString(c.getContentResolver(), Settings.Secure.ENABLED_ACCESSIBILITY_SERVICES); if (cur == null) cur = "";
                for(String s : svcs) { String mySvc = s.trim(); if(mySvc.isEmpty()) continue; boolean en = cur.contains(mySvc); if (en) cur = cur.replace(":" + mySvc, "").replace(mySvc + ":", "").replace(mySvc, ""); else cur = cur.isEmpty() ? mySvc : cur + ":" + mySvc; }
            ...
            String data = i.getDataString(); if (data != null && data.startsWith("acc://")) { toggleAcc(c, data.substring(6)); }
        }
    }
