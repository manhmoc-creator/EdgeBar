...
@Override public void onClick() {
    ...
    if (!isAccEnabled()) {
        ...
        return;
           ...
           // Toggle MorseLock — CHỈ ghi pref khi service CHƯA chạy (lúc đó không có ai
        // đọc/đảo lại giá trị). Nếu service ĐANG chạy, để chính HomescreenService là
        // nơi DUY NHẤT thực hiện việc đảo giá trị (qua TOGGLE_MORSE), tránh double-toggle
        // (bug cũ: Tile ghi pref rồi lại gửi broadcast khiến HomescreenService đảo lần 2,
        // làm giá trị bật lại ngay sau khi vừa tắt).
        boolean newState = !isMorseOn();

        if (!HomescreenService.isRunning) {
            getSharedPreferences("EdgeBarPrefs", MODE_PRIVATE)
                .edit().putBoolean("morse_mode_en", newState).apply();
            if (newState) {
            ...
            // Service đang sống -> để nó tự đảo pref + tự dọn dẹp/checkSelfStop()
